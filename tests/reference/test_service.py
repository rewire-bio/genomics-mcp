...
async def test_lookup_variant_isolates_429_and_timeout(router: Router, make_service) -> None:
    ...
    router.add(
        "GET", EUTILS + r"/esearch", httpx.Response(429, json={"error": "API rate limit exceeded"})
    )
    router.add("POST", r"gnomad", timeout)
    result = await make_service(ReferenceConfig(max_retries=1)).lookup_variant(
...
async def test_lookup_variant_end_to_end_with_real_fixtures(router: Router, make_service) -> None:
    ...
    out = await call_tool(
        make_service(ReferenceConfig(max_retries=0)),
        "lookup_variant",
    ...
    assert ("ensembl", "vep/region", 500) in {
        (e["source"], e["operation"], e.get("status_code")) for e in out["errors"]
    }
    assert out["normalization"]["errors"][0]["operation"] == "sequence/region"


async def test_multiallelic_rsid_returns_candidates_and_no_lookups(
    router: Router, make_service
) -> None:
    router.json(
        "GET",
        r"api\.ncbi\.nlm\.nih\.gov/variation/v0/refsnp/113488022",
        load_json("ncbi_refsnp_113488022_trimmed.json"),
    )
    result = await make_service().lookup_variant(
        LookupVariantRequest(variant="rs113488022", assembly="GRCh38")
    )
    assert result.status == "ambiguous"
    assert result.canonical_variant is None
    spdis = {c.spdi for c in result.normalization.candidates}
    assert spdis == {
        "NC_000007.14:140753335:A:C",
        "NC_000007.14:140753335:A:G",
...
async def test_rsid_falls_back_to_ensembl_when_ncbi_fails(router: Router, make_service) -> None:
    ...
    router.json(
        "GET",
        r"grch37\.rest\.ensembl\.org/variant_recoder/",
        load_json("ensembl_grch37_variant_recoder_rs113488022.json"),
    )
...
async def test_coding_hgvs_resolves_through_ncbi_with_exact_version(
    router: Router, make_service
) -> None:
    router.json(
        "GET",
        r"variation/v0/hgvs/.+/contextuals",
        load_json("ncbi_hgvs_contextuals_NM_004333.6.json"),
    )
    router.json(
        "GET",
        r"variation/v0/spdi/.+/all_equivalent_contextual",
        load_json("ncbi_all_equivalent_NM_004333.6.json"),
    )
...
async def test_transcript_version_remapped_by_ensembl_is_not_selected(
    router: Router, make_service
) -> None:
    ...
    router.json(
        "GET",
        r"grch37\.rest\.ensembl\.org/variant_recoder/",
        load_json("ensembl_grch37_variant_recoder_NM_004333.4.json"),
    )
    result = await make_service(ReferenceConfig(max_retries=0)).normalize_variant(
        NormalizeVariantRequest(variant="NM_004333.4:c.1799T>A", assembly="GRCh37")
    )
...
async def test_unversioned_transcript_is_rejected_without_requests(
    router: Router, make_service
) -> None:
    result = await make_service().normalize_variant(
        NormalizeVariantRequest(variant="NM_004333:c.1799T>A", assembly="GRCh38")
    )
...
async def test_protein_hgvs_is_never_selected(router: Router, make_service) -> None:
    router.json(
        "GET",
        r"grch37\.rest\.ensembl\.org/variant_recoder/",
        load_json("ensembl_grch37_variant_recoder_BRAF_p.json"),
    )
    result = await make_service().lookup_variant(
        LookupVariantRequest(variant="BRAF:p.Val600Glu", assembly="GRCh37")
    )
    assert result.status == "unresolved"
    assert result.canonical_variant is None
    assert result.normalization.candidates[0].vcf.alt == "T"
    assert any(
        "may resolve to multiple genomic locations" in w for w in result.normalization.warnings
    )
...
async def test_call_tool_validation_and_schemas(make_service) -> None:
    out = await call_tool(
        make_service(),
        "normalize_variant",
        {"variant": {"assembly": "GRCh38", "contig": "7", "ref": "A", "alt": "T"}},
    )
