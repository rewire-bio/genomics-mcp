...
from genomics_mcp.references.variants import RawAllele, ReferenceWindow, normalize_allele, structured_allele
...
async def test_private_variant_without_consent_makes_zero_requests(router: Router, make_service, kwargs) -> None:
    request = LookupVariantRequest(assembly="GRCh38", query_origin="private_file", allow_external_queries=False, **kwargs)
...
async def test_private_normalize_without_consent_is_local_only(router: Router, make_service) -> None:
...
async def test_private_normalize_with_supplied_reference_verifies_locally(router: Router, make_service) -> None:
    result = await make_service().normalize_variant(NormalizeVariantRequest(
...
async def test_explicit_consent_permits_the_request(router: Router, make_service) -> None:
    router.json("POST", r"gnomad\.broadinstitute\.org/api", load_json("gnomad_variant_7-140753336-A-T.json"))
    result = await make_service().lookup_variant(LookupVariantRequest(
        variant="7-140753336-A-T", assembly="GRCh38", use_remote_reference=False, sources=["gnomad"],
        query_origin="private_file", allow_external_queries=True))
...
async def test_short_reference_covering_indel_keeps_ref_evidence(router: Router, make_service) -> None:
    # REF "ACA" at 0-based 1007 is covered, but the CA repeat continues left of the window.
    result = await make_service().normalize_variant(NormalizeVariantRequest(
        variant={"assembly": "GRCh38", "contig": "7", "position": 1008, "ref": "ACA", "alt": "A"},
...
async def test_short_reference_with_mismatching_indel_ref_is_error(make_service) -> None:
    result = await make_service().normalize_variant(NormalizeVariantRequest(
        variant={"assembly": "GRCh38", "contig": "7", "position": 1008, "ref": "AGA", "alt": "A"},
        reference={"assembly": "GRCh38", "contig": "7", "start": 1006, "sequence": "CACAG"},
        use_remote_reference=False,
    ))
    assert result.status == "error"
...
START = ReferenceWindow(assembly="GRCh38", contig="7", start=0, sequence="AAAC", source="synthetic",
                        at_contig_start=True, at_contig_end=True)
...
async def test_contig_start_indels_are_right_anchored(position: int, ref: str, alt: str, expected) -> None:
...
async def test_deleting_whole_contig_is_unanchorable_and_explained() -> None:
    whole = ReferenceWindow(assembly="GRCh38", contig="7", start=0, sequence="AAAC", source="synthetic",
                            at_contig_start=True, at_contig_end=True)
    ...
    gone = RawAllele(assembly="GRCh38", contig="7", start=0, end=4, ref="AAAC", alt="", origin="structured")
...
def http_for(router: Router, clock: FakeClock, **kwargs) -> SourceHttp:
    ...
    return SourceHttp(client, source="test", limiter=RateLimiter(100, 1.0, clock=clock, sleep=clock.sleep),
...
async def test_streamed_body_is_cut_at_limit_without_reading_everything(router: Router, clock: FakeClock) -> None:
...
async def test_declared_length_over_limit_fails_before_reading(router: Router, clock: FakeClock) -> None:
...
async def test_redirects_are_not_followed_even_if_client_would(router: Router, clock: FakeClock) -> None:
    router.add("GET", r"example\.org/start", httpx.Response(302, headers={"location": "https://elsewhere.test/x"}))
...
async def test_stalled_source_does_not_discard_completed_results(router: Router) -> None:
    ...
    t0 = time.monotonic()
    result = await service.lookup_gene(LookupGeneRequest(gene="BRCA2", include=["identifiers", "disease", "constraint"]))
    elapsed = time.monotonic() - t0
    assert elapsed < 3.0
    assert result.status == "partial"
    assert {e.source for e in result.evidence} == {"hgnc", "opentargets"}
...
async def test_ensembl_outage_falls_back_to_same_build_only(router: Router, make_service) -> None:
    ...
    router.add("GET", r"eutils\.ncbi\.nlm\.nih\.gov/entrez/eutils/efetch\.fcgi",
...
ORACLE_START = ReferenceWindow(assembly="GRCh38", contig="7", start=0, sequence="AAACACACACAGGGGTTTT",
                               source="synthetic", at_contig_start=True)
...
async def test_shift_to_contig_start_keeps_rotation(position: int, ref: str, alt: str, expected) -> None:
