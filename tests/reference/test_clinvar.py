...
def braf_variant(assembly: str = "GRCh38") -> CanonicalVariant:
    ...
    return CanonicalVariant(
        ...
        spdi=f"{acc}:{pos - 1}:A:T",
        ncbi_canonical_spdi=f"{acc}:{pos - 1}:A:T",
    )
...
async def test_real_vcv_keeps_classification_types_separate() -> None:
    ...
    assert any(
        loc["assembly"] == "GRCh37" and loc["position_vcf"] == "140453136"
        for loc in record.data["locations"]
    )
...
async def test_real_scvs_keep_submitter_review_status_condition_and_type() -> None:
    ...
    somatic = [
        e.data for e in scvs.values() if e.data["classification_type"] == "somatic_clinical_impact"
    ]
...
async def test_somatic_conflict_and_oncogenicity_are_preserved_per_submitter() -> None:
    ...
    assert [d["clinical_impact_clinical_significance"] for d in somatic["description"]] == [
    ...
    assert evidence[0].data["submitted_classification_counts"]["somatic_clinical_impact"][
        "contributing_to_aggregate"
    ] == {
...
def clinvar_routes(router: Router) -> None:
    ...
    router.add(
        ...
        httpx.Response(200, content=load_bytes("clinvar_vcv_13961_trimmed.xml")),
    )


async def test_grch38_allele_is_matched_by_canonical_spdi_not_position(
    ...
    assert (
        evidence[0].source_release
        == "ClinVar Entrez Build260924-0125.1 (lastupdate 2026/09/24 04:57)"
    )
...
async def test_grch37_allele_is_matched_on_vcf_fields_from_vcv_xml(
    router: Router, make_service
) -> None:
    summary = load_json("clinvar_esummary_braf.json")
    router.json(
        "GET",
        EUTILS + r"/esearch\.fcgi",
        {"esearchresult": {"count": "2", "idlist": ["13961", "40389"]}},
    )
    router.json(
        "GET",
        EUTILS + r"/esummary\.fcgi",
        {
            "result": {
                "uids": ["13961", "40389"],
    ...
    assert match.matched_ids == ["13961"]
    assert [t.operation for t in match.transformations] == [
        "clinvar_position_search",
        "clinvar_vcf_match",
    ]
...
async def test_empty_efetch_is_not_found(router: Router, make_service) -> None:
    router.add(
        ...
        EUTILS + r"/efetch\.fcgi",
        httpx.Response(
            200, content=b'<?xml version="1.0" ?><ClinVarResult-Set><set/></ClinVarResult-Set>'
        ),
    )
...
async def test_full_real_vcv_maps_every_scv_with_types_separated() -> None:
    ...
    assert (
        types.count("germline"),
        types.count("somatic_clinical_impact"),
        types.count("oncogenicity"),
    ) == (22, 21, 2)
