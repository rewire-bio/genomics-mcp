...
FILES = [
    ...
    {
        "accession_id": "EGAF00000000002",
        "unencrypted_checksum": "cd" * 16,
        "unencrypted_checksum_type": "MD5",
        "filesize": 10,
        "extension": "bai",
        "has_report": False,
    ...
    {
        "accession_id": "EGAF00000000003",
        "unencrypted_checksum": "ef" * 16,
        "unencrypted_checksum_type": "MD5",
        "filesize": 10,
        "extension": "1664408722194",
        "has_report": True,
        "locations": ["ebi"],
    },
]
...
async def test_list_files_paging_and_readiness(router):
    ...
    bam, bai = p1.items
    assert (
        bam.uri == "ega://EGAF00000000001"
        and bam.format == "bam"
        and bam.access_status == "controlled"
    )
    assert bam.visibility == "private" and bam.size_bytes is None and bam.native["filesize"] == 1000
    assert (
        bam.checksums[0].value == "ab" * 16 and bam.index_uri is None
    )  # public API links no index
...
async def test_samples_keep_phenotype_values_and_dataset_link(router):
    ...
    s = page.items[0]
    assert [(p.name, p.value) for p in s.phenotypes] == [
        ("phenotype", "unknown"),
        ("biological_sex", "female"),
    ]
    assert s.native["subject_id"] == "P1" and s.phenotype_files == []
    assert (s.links[0].relation, s.links[0].kind, s.links[0].accession) == (
        "part_of",
        "dataset",
        DS,
    )
...
async def test_describe_reports_unreconciled_counts_and_omits_dac_contacts(router):
    ...
    router.add(
        "GET",
        f"{META}/datasets/{DS}/studies",
        json_response([{"accession_id": "EGAS00000000001", "title": "St"}]),
    )
    ...
    router.add(
        "GET",
        f"{META}/policies/EGAP00000000001",
        {"accession_id": "EGAP00000000001", "dac_accession_id": "EGAC00000000001"},
    )
    router.add(
        "GET",
        f"{META}/dacs/EGAC00000000001",
        {
...
async def test_direct_password_grant_secrets_are_redacted(router, tmp_path):
    router.add(
        ...
        json_response(
            {
                "error": "literal synthetic-password-value echo",
                "error_description": "synthetic-client-secret",
            },
            400,
        ),
    )
...
async def test_invalid_grant_code_kept_and_file_loaded_grant(router, tmp_path):
    (tmp_path / "server.json").write_text(
        json.dumps({"url_auth": TOKEN_URL, "client_secret": "file-client-secret-x"})
    )
    (tmp_path / "cred.json").write_text(
        json.dumps({"username": "u", "password": "file-password-value-x"})
    )
    router.add("POST", TOKEN_URL, json_response({"error": "invalid_grant"}, 401))
    auth = EgaAuth(
        grant=EgaPasswordGrant.from_pyega3_files(tmp_path / "server.json", tmp_path / "cred.json")
    )
    with pytest.raises(UnauthorizedError) as ei:
...
async def test_token_is_cached_and_never_serialised(router):
    router.add(
        "POST",
        TOKEN_URL,
        json_response({"access_token": "synthetic-access-token-abc", "expires_in": 3600}),
    )
    auth = EgaAuth(
        grant=EgaPasswordGrant(
            username="u",
            password=SecretStr("pw-synthetic-1"),
...
def authed_file_routes(router: Router, data: bytes, *, name: str, md5: str | None = None) -> None:
    ...
    router.add(
        "GET",
        f"{DATA}/metadata/files/EGAF00000000001",
        {
            "fileId": "EGAF00000000001",
            "datasetId": [DS],
            "indexFileId": "EGAF00000000002",
            "displayFileName": name,
            "fileSize": len(data) + 16,
            "plainChecksum": md5 or hashlib.md5(data).hexdigest(),
...
async def test_fetch_file_verifies_md5_uses_range_and_safe_name(router, tmp_path):
    ...
    ws = tmp_path / "ws"
    art = await client(router, token_auth()).fetch_file(
        "EGAF00000000001", workspace=ws, budget_bytes=1000
    )
    p = Path(art.path)
    assert p.parent == ws.resolve() and p.read_bytes() == data and art.checksum_verified  # noqa: ASYNC240 - small local test file
...
async def test_fetch_file_md5_mismatch_removes_file(router, tmp_path):
    ...
    with pytest.raises(UpstreamError):
        await client(router, token_auth()).fetch_file(
            "EGAF00000000001", workspace=tmp_path, budget_bytes=1000
        )
...
async def test_fetch_file_over_budget_never_downloads(router, tmp_path):
    ...
    with pytest.raises(BudgetExceededError):
        await client(router, token_auth()).fetch_file(
            "EGAF00000000001", workspace=tmp_path, budget_bytes=100
        )
