...
def gsm(acc: str, channels: int = 1) -> str:
    lines = [
        f"^SAMPLE = {acc}",
        f"!Sample_title = {acc} title",
        f"!Sample_channel_count = {channels}",
...
def test_ftp_to_https_only_for_ncbi_host():
    assert to_https("ftp://ftp.ncbi.nlm.nih.gov/geo/x.bw") == (
        "https://ftp.ncbi.nlm.nih.gov/geo/x.bw",
        True,
    )
...
async def test_sample_characteristics_verbatim_links_typed_contacts_dropped(router):
    ...
    assert links == {
        ("part_of", "study", "GSE1000"),
        ("biosample", "sample", "SAMN1"),
...
async def test_search_pages_natively(router):
    router.add(
        ...
        json_response({"esearchresult": {"count": "3", "idlist": ["200001"]}}),
    )
    router.add(
        "GET",
        f"{EUTILS}/esummary.fcgi",
        json_response(
            {
                "result": {
                    "uids": ["200001"],
                    "200001": {"accession": "GSE1", "title": "t", "n_samples": 2},
                }
            }
        ),
    )
    ...
    assert (
        q["term"] == "(CTCF liver) AND gse[Entry Type]"
        and q["retstart"] == "0"
        and q["retmax"] == "1"
    )
