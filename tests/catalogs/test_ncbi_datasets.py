...
ACC = "GCF_000819615.1"
REPORT = {"reports": [{"accession": ACC, "current_accession": ACC, "paired_accession": "GCA_000819615.1",
                       "source_database": "SOURCE_DATABASE_REFSEQ",
                       "organism": {"tax_id": 1, "organism_name": "phage"},
                       "assembly_info": {"assembly_name": "ViralProj14015", "assembly_status": "current",
                                         "bioproject_accession": "PRJNA1"}}], "total_count": 1}
...
def package(members: dict[str, bytes], md5: dict[str, str] | None = None) -> bytes:
    ...
    with zipfile.ZipFile(buf, "w") as z:
        ...
        sums = md5 if md5 is not None else {n: hashlib.md5(d).hexdigest() for n, d in members.items()}
        z.writestr("md5sum.txt", "".join(f"{v}  {k}\n" for k, v in sums.items()))
    return buf.getvalue()


def summary_route(router: Router, size_mb: float = 0.01) -> None:
    router.add("GET", f"{BASE}/genome/accession/{ACC}/download_summary", json_response(
        {"record_count": 1, "available_files": {"all_genomic_fasta": {"file_count": 1, "size_mb": size_mb},
...
async def test_describe_keeps_versions_and_insdc_linkage(router):
    router.add("GET", f"{BASE}/genome/accession/{ACC}/dataset_report",
    ...
    assert {(link.relation, link.kind) for link in d.dataset.links} == {("paired_assembly", "reference"),
                                                                        ("bioproject", "study")}
...
async def test_prepare_genome_fasta_verifies_extracts_and_indexes(router, tmp_path):
    ...
    router.add("GET", f"{BASE}/genome/accession/{ACC}/download",
    ...
    assert Path(art.path).read_bytes() == FASTA and art.checksum_verified and art.format == "fasta"
    assert Path(art.index_path).read_text().startswith("NC_001422.1\t14\t")
    assert sorted(p.name for p in tmp_path.iterdir()) == [f"{ACC}.fna", f"{ACC}.fna.fai"]  # ZIP removed
...
async def test_prepare_ignores_traversal_members(router, tmp_path):
    summary_route(router)
    router.add("GET", f"{BASE}/genome/accession/{ACC}/download", httpx.Response(200, content=package({
...
async def test_taxon_search_uses_native_page_token(router):
    def report(req: httpx.Request) -> httpx.Response:
        ...
        return json_response({"reports": REPORT["reports"], "total_count": 2,
