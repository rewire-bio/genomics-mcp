...
def file_obj(**kw) -> dict:
    base = {"@id": "/files/ENCFF000AAA/", "accession": "ENCFF000AAA", "file_format": "bigBed",
            "assembly": "mm9", "file_size": 1000, "md5sum": "a" * 32,
            "href": "/files/ENCFF000AAA/@@download/ENCFF000AAA.bigBed", "status": "archived",
            "dataset": "/experiments/ENCSR000AAA/", "cloud_metadata": {"url": S3, "file_size": 1000},
...
def test_file_uses_unsigned_url_drops_sas_and_types_links():
    ...
    assert f.uri == S3 and f.native["href"] == f"{BASE}/files/ENCFF000AAA/@@download/ENCFF000AAA.bigBed"
    ...
    assert {(link.relation, link.kind, link.accession) for link in f.relationships} == {
        ("part_of", "experiment", "ENCSR000AAA"), ("derived_from", "file", "ENCFF000BBB")}


def test_annotation_dataset_is_not_called_an_experiment():
    f = EncodeClient.__new__(EncodeClient)._file(file_obj(dataset="/annotations/ENCSR000ABC/"), PROV)
...
@pytest.mark.parametrize(("resp", "state"), [
    (httpx.Response(206, content=b"\xeb\xf2\x89\x87" + b"\0" * 60,
                    headers={"content-range": "bytes 0-63/1000"}), "ready"),
    (httpx.Response(200, content=b"\xeb\xf2\x89\x87" + b"\0" * 60), "download_required"),
    (httpx.Response(206, content=b"<html>" + b"\0" * 58, headers={"content-range": "bytes 0-63/1000"}),
     "unsupported"),
    (httpx.Response(206, content=b"\xeb\xf2\x89\x87" + b"\0" * 60,
                    headers={"content-range": "bytes 0-63/999"}), "unknown"),
])
...
async def test_list_samples_follows_replicate_library_biosample(router):
    bio = {"accession": "ENCBS000AAA", "sex": "female", "age": "14", "age_units": "day",
           "summary": "s", "organism": {"scientific_name": "Mus musculus", "taxon_id": "10090"},
           "biosample_ontology": {"term_name": "liver", "term_id": "UBERON:0002107"},
           "donor": {"accession": "ENCDO000AAA", "@id": "/mouse-donors/ENCDO000AAA/"}}
    exp = {"@id": "/experiments/ENCSR000AAA/", "accession": "ENCSR000AAA", "replicates": [
        {"@id": "/replicates/r1/", "biological_replicate_number": 1, "technical_replicate_number": 1,
         "library": {"accession": "ENCLB1", "biosample": bio}},
        {"@id": "/replicates/r2/", "biological_replicate_number": 1, "technical_replicate_number": 2,
    ...
    (s,) = page.items
    assert s.taxon_id == 10090 and [r["library"] for r in s.native["replicates"]] == ["ENCLB1", "ENCLB2"]
    phen = {p.name: (p.value, p.ontology_term) for p in s.phenotypes}
    assert phen["sex"] == ("female", None) and phen["biosample_term_name"] == ("liver", "UBERON:0002107")
    assert {(link.relation, link.kind, link.accession) for link in s.links} == {
        ("replicate_of", "experiment", "ENCSR000AAA"), ("donor", "individual", "ENCDO000AAA")}
...
async def test_list_files_pages_sorted_accessions(router):
    router.add("GET", f"{BASE}/ENCSR000AAA/", json_response({"@id": "/experiments/ENCSR000AAA/",
    ...
    def search(req: httpx.Request) -> httpx.Response:
        ...
        return json_response({"@graph": [file_obj(accession=a, **{"@id": f"/files/{a}/"}) for a in wanted]})
    ...
    assert [f.accession for f in p1.items + p2.items] == ["ENCFF000AAA", "ENCFF000BBB", "ENCFF000CCC"]
