...
TOOLS: dict[str, ToolSpec] = {
    ...
    for spec in (
        ToolSpec(
            ...
            ResolveIdentifierRequest, ResolveIdentifierResult, "resolve_identifier",
        ),
        ToolSpec(
            "normalize_variant",
            ...
            NormalizeVariantRequest, NormalizeVariantResult, "normalize_variant",
        ),
        ToolSpec(
            "lookup_variant",
            ...
            LookupVariantRequest, LookupVariantResult, "lookup_variant",
        ),
        ToolSpec(
            "lookup_gene",
            ...
            LookupGeneRequest, LookupGeneResult, "lookup_gene",
        ),
        ToolSpec(
            "lookup_protein",
            ...
            LookupProteinRequest, LookupProteinResult, "lookup_protein",
        ),
    )
}
...
def source_status(service: ReferenceService) -> list[dict[str, Any]]:
    ...
    for name, info in SOURCES.items():
        ...
        if name == "alphagenome_atlas":
            configured = service._atlas_transport is not None or service.config.atlas_api_key is not None
...
async def call_tool(service: ReferenceService, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    spec = TOOLS.get(name)
    if spec is None:
    ...
    try:
        ...
        problems = "; ".join(
            f"{'.'.join(str(p) for p in e['loc']) or 'arguments'}: {e['msg']}" for e in exc.errors()[:10]
        )
        result = ToolResult(
            tool=name,
            status="error",
            query={"argument_keys": sorted(arguments)},
            errors=[SourceError(source="local", operation="validate", kind="invalid_input", message=problems)],
        )
