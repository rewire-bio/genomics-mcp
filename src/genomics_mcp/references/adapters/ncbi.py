        ...
        *,
        ...
        if accession is None:
            raise failure(
                ...
                "efetch/fasta",
                "unsupported",
                f"no RefSeq accession for {assembly} {contig}",
            )
        if start < 0 or end <= start:
            raise failure(
                ...
                "efetch/fasta",
                "invalid_input",
                "empty or negative sequence interval",
            )
        params = {
            "db": "nuccore",
            "id": accession,
        ...
        try:
            ...
            raise failure(
                "ncbi_nuccore", "efetch/fasta", "invalid_response", "response was not FASTA"
            ) from None
        if not header.startswith(accession):
            raise failure(
                ...
                "efetch/fasta",
                "invalid_response",
                f"FASTA header is not {accession}",
            )
        if not seq:
...
class NcbiVariationClient:
    ...
    async def refsnp(
        self, rsid: str, assembly: Assembly, *, deadline: float | None = None, _hops: int = 0
    ) -> RefSnpResult:
        number = rsid.lower().removeprefix("rs")
        data = await self.http.get_json(
            f"{VARIATION}/refsnp/{quote(number)}", operation="refsnp", deadline=deadline
        )
        ...
        if "merged_snapshot_data" in body and "primary_snapshot_data" not in body:
            ...
            raise failure(
                ...
                "refsnp",
                "not_found",
                f"rs{number} is merged into {merged}; not followed",
            )
        ...
        if not isinstance(snapshot, dict):
            ...
            raise failure(
                ...
                "refsnp",
                "not_found",
                f"rs{number} has no current placement ({state})",
            )
        placements: list[SpdiPlacement] = []
        ...
        return RefSnpResult(
            ...
            notes=[f"dbSNP variant_type: {snapshot.get('variant_type')}"]
            if snapshot.get("variant_type")
            else [],
            last_update_build=str(body["last_update_build_id"])
            if body.get("last_update_build_id")
            else None,
            last_update_date=body.get("last_update_date"),
        )

    async def hgvs_to_genomic(
        self, hgvs: str, assembly: Assembly, *, deadline: float | None = None
    ) -> tuple[list[SpdiPlacement], list[str]]:
        ...
        body = require_dict(
            self.http, self.http.decode_json(response, "hgvs/contextuals"), "hgvs/contextuals"
        )
        payload = body.get("data") or {}
        notes = (
            [f"NCBI input_hgvs_validity: {payload['input_hgvs_validity']}"]
            if payload.get("input_hgvs_validity")
            else []
        )
        ...
        for s in spdis:
            if refseq_to_contig(s.get("seq_id", "")):
                ...
                genomic.extend(
                    e
                    for e in (eq_body.get("data") or {}).get("spdis") or []
