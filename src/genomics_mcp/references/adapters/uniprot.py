...
DEFAULT_FEATURE_TYPES = (
    "Chain", "Signal", "Transit peptide", "Propeptide", "Domain", "Region", "Motif", "Repeat", "Zinc finger",
    "DNA binding", "Coiled coil", "Compositional bias", "Active site", "Binding site", "Site", "Transmembrane",
    "Topological domain", "Intramembrane", "Modified residue", "Glycosylation", "Disulfide bond", "Cross-link",
    "Lipidation",
)
MAX_FEATURES = 150
...
class UniprotClient:
    ...
    async def entry(self, accession: str, *, deadline: float | None = None, _hops: int = 0) -> tuple[dict[str, Any], str | None, list[Transformation]]:
        m = ACCESSION_RE.match(accession.strip().upper())
        if not m:
            raise failure("uniprot", "entry", "invalid_input", f"{accession!r} is not a UniProtKB accession")
        base_acc = m.group(1)
        transformations: list[Transformation] = []
        if m.group(2):
            transformations.append(
                Transformation(operation="isoform_to_entry", source="uniprot",
                               detail=f"isoform {accession} reported through its entry {base_acc}; isoform-specific sequence not selected")
            )
        response = await self.http.request(
            "GET", f"{BASE}/{quote(base_acc)}.json", operation="entry", deadline=deadline, accept_status=(200, 303),
        )
        ...
        if body.get("entryType") == "Inactive":
            ...
            if rtype == "MERGED" and len(targets) == 1 and _hops == 0:
                ...
                return entry, release2, transformations + [
                    Transformation(operation="accession_merged", source="uniprot",
                                   detail=f"{base_acc} is inactive (merged) in UniProtKB; reporting {targets[0]}",
                                   before={"accession": base_acc}, after={"accession": targets[0]})
                ] + steps
            raise failure("uniprot", "entry", "not_found",
        ...
        if returned and returned != base_acc:
            ...
            transformations.append(
                Transformation(operation="accession_redirected", source="uniprot",
                               detail=f"UniProt answered {base_acc} with entry {returned}",
                               before={"accession": base_acc}, after={"accession": returned})
            )
        return body, release, transformations

    def entry_evidence(self, entry: dict[str, Any], release: str | None, transformations: list[Transformation],
                       *, include_all_features: bool = False) -> Evidence:
        ...
        for c in entry.get("comments") or []:
            ...
            if ctype == "FUNCTION":
                for t in c.get("texts") or []:
                    ...
                    if cut:
                        truncation.append(Truncation(field="data.function", returned=1500, reason="text shortened"))
                ...
                d = c["disease"]
                disease.append({k: v for k, v in {
                    "id": d.get("diseaseId"), "acronym": d.get("acronym"),
                    "xref": f"{(d.get('diseaseCrossReference') or {}).get('database')}:{(d.get('diseaseCrossReference') or {}).get('id')}"
                    if d.get("diseaseCrossReference") else None,
                }.items() if v})
        features_all = [f for f in entry.get("features") or [] if isinstance(f, dict)]
        counts = Counter(f.get("type") for f in features_all)
        selected = features_all if include_all_features else [f for f in features_all if f.get("type") in DEFAULT_FEATURE_TYPES]
        features = []
        for f in selected[:MAX_FEATURES]:
            ...
            if f.get("alternativeSequence"):
                ...
                item["change"] = f"{a.get('originalSequence')}>{'/'.join(a.get('alternativeSequences') or [])}"
            features.append({k: v for k, v in item.items() if v is not None})
        if len(selected) > MAX_FEATURES:
            truncation.append(Truncation(field="data.features", returned=MAX_FEATURES, available=len(selected),
        ...
        for x in entry.get("uniProtKBCrossReferences") or []:
            ...
            if db in ("Ensembl", "RefSeq", "MANE-Select", "HGNC", "MIM", "GeneID", "CCDS"):
                ...
                row = {"id": x.get("id"), **{k: v for k, v in props.items() if k in ("ProteinId", "GeneId", "NucleotideSequenceId", "RefSeqNucleotideId", "RefSeqProteinId")}}
                xrefs.setdefault(db, []).append(row if len(row) > 1 else x.get("id"))
        pdb_count = sum(1 for x in entry.get("uniProtKBCrossReferences") or [] if x.get("database") == "PDB")
        ...
        data = {
            ...
            "genes": genes,
            "organism": {"taxon_id": (entry.get("organism") or {}).get("taxonId"),
                         "name": (entry.get("organism") or {}).get("scientificName")},
            "sequence": {"length": seq.get("length"), "mass_da": seq.get("molWeight"), "crc64": seq.get("crc64"),
                         "md5": seq.get("md5"), "version": audit.get("sequenceVersion")},
        ...
        if not include_all_features:
            limitations.append("Natural variant, mutagenesis, conflict and secondary-structure features are counted but not listed.")
        limitations.append("Feature coordinates are 0-based half-open on the canonical UniProt sequence.")
        return Evidence(
            source="uniprot",
            evidence_type="protein_entry",
            source_record_id=accession,
            source_record_version=str(audit.get("entryVersion")) if audit.get("entryVersion") else None,
            ...
            + [Transformation(operation="uniprot_to_zero_based", source="local",
                              detail="UniProt 1-based feature start written as 0-based half-open start")],
            limitations=limitations,
            truncation=truncation,
        )

    async def search(self, query: str, *, size: int = 25, deadline: float | None = None) -> ProteinSearch:
        response = await self.http.request(
            "GET", f"{BASE}/search", operation="search",
        ...
        for r in results:
            ...
            candidates.append(IdentifierCandidate(
                entity_type="protein", identifier=acc, label=name, source="uniprot",
        ...
        return ProteinSearch(candidates=candidates, reviewed=reviewed, total=total, release=_release(response), notes=notes)
