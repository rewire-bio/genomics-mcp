...
class AtlasTransport(Protocol):
    ...
    async def get_dense_variant_scores(
        self, *, chromosome: str, position: int, reference_bases: str, alternate_bases: str, filter: str, timeout: float
    ) -> Any: ...

    async def list_variant_scores_metadata(self, *, timeout: float) -> Any: ...
...
class GrpcAtlasTransport:
    ...
    def __init__(self, api_key: SecretStr, *, address: str = DEFAULT_ADDRESS):
        try:
            ...
            raise failure(
                "alphagenome_atlas", "connect", "unsupported",
        ...
        channel = grpc.aio.secure_channel(
            address, grpc.ssl_channel_credentials(), options=(("grpc.service_config", service_config),)
        )
    ...
    async def get_dense_variant_scores(
        ...
        request = self._pb.GetDenseVariantScoresRequest(
            variant=self._dna.Variant(
                chromosome=chromosome, position=position, reference_bases=reference_bases, alternate_bases=alternate_bases
            ),
            organism=self._dna.ORGANISM_HOMO_SAPIENS,
            filter=filter,
        )
        return await self._stub.GetDenseVariantScores(request, metadata=self._metadata, timeout=timeout)

    async def list_variant_scores_metadata(self, *, timeout: float) -> Any:
        request = self._pb.ListVariantScoresMetadataRequest(organism=self._dna.ORGANISM_HOMO_SAPIENS)
        return await self._stub.ListVariantScoresMetadata(request, metadata=self._metadata, timeout=timeout)
...
def build_filter(requested_scorers: Sequence[str]) -> str:
    ...
    for name in requested_scorers:
        if not SCORER_NAME_RE.match(name):
            raise failure("alphagenome_atlas", "get_dense_variant_scores", "invalid_input", f"invalid scorer name {name!r}")
...
def decode_floats(raw: bytes, shape: Sequence[int]) -> list[float]:
    ...
    if len(raw) != 4 * n:
        raise failure("alphagenome_atlas", "get_dense_variant_scores", "invalid_response",
...
class AtlasClient:
    ...
    def _require(self, operation: str) -> AtlasTransport:
        if self.transport is None:
            raise failure(
                "alphagenome_atlas", operation, "not_configured",
                "AlphaGenome Atlas is disabled: no API key was configured explicitly. See " + INFO.terms_url,
            )
    ...
    def _map_error(self, exc: Exception, operation: str) -> SourceFailure:
        ...
        if isinstance(exc, asyncio.TimeoutError):
            return failure("alphagenome_atlas", operation, "timeout", "no response before the deadline", retryable=True)
        ...
        return failure("alphagenome_atlas", operation, kind, message,  # type: ignore[arg-type]
    ...
    async def _call(self, operation: str, deadline: float | None, fn: Any, **kwargs: Any) -> Any:
        try:
            ...
            raise failure("alphagenome_atlas", operation, "rate_limited", str(exc), retryable=True) from None
        remaining = None if deadline is None else deadline - self._clock()
        if remaining is not None and remaining <= 0:
            raise failure("alphagenome_atlas", operation, "timeout", "deadline exhausted before request")
        timeout = self.timeout if remaining is None else max(0.1, min(self.timeout, remaining))
        try:
            return await asyncio.wait_for(fn(timeout=timeout, **kwargs), timeout + 1.0)
    ...
    async def scorers(self, *, deadline: float | None = None) -> list[dict[str, Any]]:
        ...
        if self._scorers is None:
            response = await self._call("list_variant_scores_metadata", deadline, transport.list_variant_scores_metadata)
            out = []
            for item in getattr(response, "variant_scorer_metadata", []) or []:
                ...
                out.append({"name": info.name, "is_signed": bool(info.is_signed), "track_count": track_count})
            self._scorers = out
    ...
    async def variant_scores(
        ...
        *,
        ...
        if variant.assembly != "GRCh38":
            raise failure("alphagenome_atlas", operation, "unsupported",
        ...
        response = await self._call(
            operation, deadline, transport.get_dense_variant_scores,
            chromosome=chromosome, position=variant.vcf.pos, reference_bases=variant.vcf.ref,
            alternate_bases=variant.vcf.alt, filter=filter_text,
        )
        returned = getattr(response, "variant", None)
        if returned is not None and (
            returned.chromosome, int(returned.position), returned.reference_bases, returned.alternate_bases
        ) != (chromosome, variant.vcf.pos, variant.vcf.ref, variant.vcf.alt):
            raise failure("alphagenome_atlas", operation, "invalid_response", "Atlas returned a different variant")
        scorers = []
        ...
        return Evidence(
            ...
            data={
                ...
                "variant": {"chromosome": chromosome, "position_1based": variant.vcf.pos,
                            "reference_bases": variant.vcf.ref, "alternate_bases": variant.vcf.alt},
                "scoring_interval_0based": interval,
                "requested_scorers": list(requested_scorers or []),
                "scorers": scorers,
            },
            transformations=[
                Transformation(operation="to_atlas_variant", detail=f"contig {variant.contig} written as {chromosome}; "
                               "position is 1-based as required by the Atlas Variant message"),
                Transformation(operation="decode_scores", detail="float32 row-major score buffers decoded; values beyond "
    ...
    def _decode_score(self, score: Any, max_values: int, truncation: list[Truncation]) -> dict[str, Any]:
        ...
        for md in getattr(score, "metadata", []) or []:
            ...
            if kind == "gene_scorers":
                rows = [
                    f"{g.gene_id}" + (f" ({g.name})" if _has(g, "name") else "") for g in md.gene_scorers.metadata
                ]
            elif kind == "tracks":
        ...
        top = [{**label(i), "score": values[i], **({"quantile": calibrated[i]} if calibrated else {})} for i in ranked]
        name = info.name
        if len(values) > max_values:
            truncation.append(Truncation(field=f"scorers[{name}].values", returned=max_values, available=len(values),
