...
class OpenTargetsClient:
    ...
    async def associations(self, ensembl_gene_id: str, *, size: int = 25, deadline: float | None = None) -> Evidence:
        response = await self.http.request(
            "POST", API, operation="target_associations",
            json_body={"query": ASSOCIATIONS_QUERY, "variables": {"ensemblId": ensembl_gene_id, "size": size}},
        ...
        if not isinstance(body, dict):
            raise failure("opentargets", "target_associations", "invalid_response", "GraphQL response is not an object")
        if body.get("errors") and not body.get("data"):
            msg = "; ".join(str(e.get("message"))[:200] for e in body["errors"] if isinstance(e, dict))
            raise failure("opentargets", "target_associations", "upstream", msg or "GraphQL error")
        data = body.get("data") or {}
        target = data.get("target")
        if target is None:
            raise failure("opentargets", "target_associations", "not_found", f"no Open Targets target {ensembl_gene_id}")
        ...
        for r in assoc.get("rows") or []:
            ...
            rows.append({
                ...
                "datatype_scores": {d.get("id"): d.get("score") for d in r.get("datatypeScores") or []},
            })
        count = assoc.get("count")
        truncation = []
        if isinstance(count, int) and count > len(rows):
            truncation.append(Truncation(field="data.associations", returned=len(rows), available=count,
        ...
        return Evidence(
            ...
            source_release=f"Open Targets Platform data {data_v}" + (f" (API {api_v})" if api_v else "") if data_v else (
                f"Open Targets API {api_v}" if api_v else None),
            terms_url=INFO.terms_url,
            data={
                "target": {"id": target.get("id"), "symbol": target.get("approvedSymbol"),
                           "name": target.get("approvedName"), "biotype": target.get("biotype")},
