...
from .http import RateLimiter, SourceFailure, SourceHttp
from .models import (
    Assembly,
    CanonicalVariant,
    Evidence,
...
# Per-call source selection (e.g. the MCP `sources` argument); None means all enabled sources.
_CALL_SOURCES: ContextVar[frozenset[str] | None] = ContextVar(
    "reference_call_sources", default=None
)


@contextmanager
def selected_sources(sources: Iterable[str] | None) -> Iterator[None]:
    """Restrict source use for the enclosed call. Applies to every HTTP request of the call."""
    token = _CALL_SOURCES.set(frozenset(sources) if sources is not None else None)
    try:
        yield
    finally:
...
class ReferenceConfig:
    ...
    source_deadlines: dict[str, float] = field(default_factory=dict)
    """Whole-call budget per source (e.g. from configured source timeouts); capped by the call deadline."""
    requests_per_minute: dict[str, float] = field(default_factory=dict)
    """Configured slow-downs; never raises a source above its documented limit."""
...
async def _guard[T](
    ...
    try:
        ...
        return await asyncio.wait_for(awaitable, max(0.0, budget_s))
    except TimeoutError:
        col.error(
            SourceError(
                source=source,
        ...
        col.error(
            SourceError(source=source, operation=operation, kind="invalid_input", message=str(exc))
        )
...
class ReferenceService:
    def __init__(
        ...
        *,
        ...
        def limiter(rate: int, per: float, source: str | None = None) -> RateLimiter:
            rpm = cfg.requests_per_minute.get(source) if source else None
            if rpm is not None and rpm < rate * 60.0 / per:
                # Configured slow-down: at most rpm per minute, never faster than documented.
                return RateLimiter(max(1, int(rpm)), 60.0, clock=clock, sleep=sleep)
            return RateLimiter(rate, per, clock=clock, sleep=sleep)

        ncbi_limiter = limiter(ncbi_rate, 1.0, "clinvar")
        identity = NcbiIdentity(api_key=cfg.ncbi_api_key, email=cfg.ncbi_email)
        self.hgnc = HgncClient(http("hgnc", limiter(10, 1.0, "hgnc")))
        self.ensembl = EnsemblClient(http("ensembl", limiter(10, 1.0, "ensembl")))
        ...
        self.gnomad = GnomadClient(http("gnomad", limiter(10, 60.0, "gnomad")))
        self.uniprot = UniprotClient(http("uniprot", limiter(10, 1.0, "uniprot")))
        self.opentargets = OpenTargetsClient(http("open_targets", limiter(5, 1.0, "open_targets")))
    ...
    def _deadline(self, seconds: float | None = None) -> float:
        budget = (
            ...
            if seconds is None
            else min(seconds, self.config.deadline_seconds)
        )
        return self.clock() + max(0.0, budget)

    def _remaining(self, deadline: float, source: str | None = None) -> float:
        left = max(0.0, deadline - self.clock())
        own = self.config.source_deadlines.get(source) if source else None
        return left if own is None else min(left, own)

    def _enabled(self, source: str) -> bool:
        if self.config.enabled_sources is not None and source not in self.config.enabled_sources:
            return False
        selected = _CALL_SOURCES.get()
    ...
    def atlas(self) -> AtlasClient:
        if self._atlas is None:
            ...
            self._atlas = AtlasClient(
                transport,
                limiter=self._atlas_limiter,
                timeout=self.config.timeout("alphagenome_atlas"),
                secret=self.config.atlas_api_key,
                clock=self.clock,
            )
        return self._atlas

    def _finish(self, result: ToolResult, col: _Collector) -> None:
        result.sources_consulted = list(dict.fromkeys(col.sources))
    ...
    # --------------------------------------------------------- normalization
    def _supplied_window(
        self, request: NormalizeVariantRequest, assembly: Assembly, contig: str
        ...
        if (s_asm, s_contig) != (assembly, contig):
            raise VariantInputError(
                "supplied reference sequence is for a different assembly/contig; no liftover is applied"
            )
        return ReferenceWindow(
            assembly=assembly,
            contig=contig,
            start=supplied.start,
            sequence=supplied.sequence.upper(),
    ...
    async def _fetched_window(
        self,
        request: NormalizeVariantRequest,
        assembly: Assembly,
        contig: str,
        start: int,
        end: int,
        ...
        if col.reference_provider is not None:
            providers.append(("local_reference", col.reference_provider.sequence))
    ...
    async def _normalize_raw(
        ...
        can_fetch = col.reference_provider is not None or (
            request.use_remote_reference and col.network_allowed
        )
            ...
            if supplied.covers(raw.start, raw.end):
                try:
                    ...
                    if not can_fetch:
                        ...
                        return outcome.variant, outcome.transformations, outcome.limitations
                    col.warnings.append(
                        "Supplied reference verified REF; more sequence was fetched to shift the indel."
                    )
            else:
                col.warnings.append(
                    "Supplied reference sequence does not cover the variant; it was not used."
                )
        pad = REFERENCE_PAD
        while can_fetch:
            ...
            if not window.covers(raw.start, raw.end):
                col.warnings.append(
                    f"{window.source} did not return the full variant interval; it was not used."
                )
                break
            try:
    ...
    async def normalize_variant(
        ...
        *,
        ...
        col = _Collector(reference_provider=reference_provider or self.reference_provider)
    ...
    async def _normalize(
        self, request: NormalizeVariantRequest, deadline: float, col: _Collector
    ) -> NormalizeVariantResult:
        variant = request.variant
        kind = "structured" if isinstance(variant, GenomicAlleleInput) else detect_kind(variant)
        result = NormalizeVariantResult(
            tool="normalize_variant", status="error", query={}, input_kind=kind
        )
        ...
        try:
            ...
            col.error(
                SourceError(
                    source="local", operation="parse", kind="invalid_input", message=str(exc)
                )
            )
            return result
        if raws is None:
        ...
        for raw in raws:
            ...
            col.transformations.extend(
                t if not prefix else t.model_copy(update={"detail": prefix + t.detail})
                for t in steps
            )
        ...
        if mismatch:
            col.error(
                SourceError(
                    source=mismatch[0].reference_check.source or "local",
        ...
        if len(alleles) == 1 and len(raws) == 1:
            ...
            col.warnings.append(
                f"Input describes {len(alleles)} alleles; each is normalized separately and none was selected."
            )
        return result
    ...
    async def _raw_alleles(
        ...
        if kind == "hgvs_genomic":
            ...
            assert m
            if m.group("acc").upper().startswith("NC_") or not m.group("acc").upper().startswith(
                ("NG_", "LRG_")
            ):
    ...
    async def _remote_hgvs(
        ...
        if not col.network_allowed:
            raise VariantInputError(
                "external resolution is not permitted for this private-file query"
            )
        m = HGVS_RE.match(text)
        assert m
        accession = m.group("acc")
        if m.group("paren"):
            col.transformations.append(
                Transformation(
                    operation="drop_hgvs_gene_label",
                    detail=f"gene label ({m.group('paren')}) removed from the HGVS reference",
                )
            )
        ...
        for source in order:
            ...
            try:
                if source == "ncbi_variation":
                    if not is_refseq or not self._enabled("ncbi_variation"):
                        continue
                    found, extra = await self.ncbi_variation.hgvs_to_genomic(
                        text, assembly, deadline=deadline
                    )
                    placements = [
                        (p.accession, p.contig, p.position, p.deleted, p.inserted, [])
                        for p in found
                    ]
                    notes = extra
                else:
                    placements, notes = await self._recoder_placements(
                        text, assembly, deadline, accession
                    )
            except SourceFailure as exc:
                col.error(exc.error)
        ...
        if len(unique) == 1:
            ...
            raw.transformations.insert(
                0,
                Transformation(
                    operation="transcript_to_genome"
                    if kind != "hgvs_genomic"
                    else "hgvs_to_chromosome",
                    source=used,
                    detail=f"{text} mapped to {assembly} chromosome {p[1]} by {used}",
                    before={"hgvs": text},
                    after={"spdi": f"{p[0]}:{p[2]}:{p[3]}:{p[4]}"},
                ),
            )
            return [raw]
        if not unique:
            col.warnings.append(
                f"{used} did not place {text} on {assembly}; no liftover or remapping is attempted"
            )
            result.status = "unresolved"
            return None
        ...
        result.status = "ambiguous"
        col.warnings.append(
            f"{text} maps to {len(unique)} genomic alleles on {assembly}; none was selected"
        )
        return None
    ...
    async def _recoder_placements(
        ...
        for item in items:
            ...
            for entry in item.values():
                ...
                named = [
                    h
                    for key in ("hgvsc", "hgvsn", "hgvsg")
                    for h in entry.get(key) or []
                    if isinstance(h, str)
                ]
                caveats: list[str] = []
    ...
    @staticmethod
    def _raw_from_placement(
        p: tuple[str, str, int, str, str, list[str]], assembly: Assembly, source: str
    ) -> RawAllele:
        _acc, contig, start, deleted, inserted, _ = p
        return RawAllele(
            assembly=assembly,
            contig=contig,
            start=start,
    ...
    def _placement_candidate(
        ...
        vcf = (
            VcfRepresentation(contig=contig, pos=start + 1, ref=deleted, alt=inserted)
        ...
        return VariantCandidate(
            ...
            contig=contig,
            vcf=vcf,
            spdi=f"{acc}:{start}:{deleted}:{inserted}" if acc.startswith("NC_") else None,
            notes=list(caveats),
        )
    ...
    async def _rsid(
        ...
        if not placements:
            col.warnings.append(
                f"{text} has no {assembly} chromosome placement in {used}; no liftover is applied"
            )
            result.status = "unresolved"
            return None
        if len(placements) == 1:
            ...
            raw = RawAllele(
                ...
                contig=p.contig,
                start=p.position,
                end=p.position + len(p.deleted),
                ref=p.deleted,
                alt=p.inserted,
                origin=used,
            )
            raw.transformations.append(
                Transformation(
                    operation="rsid_to_allele",
                    source=used,
        ...
        result.candidates = [
            VariantCandidate(
                description=f"{text} allele {p.deleted or '-'}>{p.inserted or '-'}",
                source=used,
                assembly=assembly,
                contig=p.contig,
                spdi=p.spdi,
                hgvs=[p.hgvs] if p.hgvs else [],
                vcf=VcfRepresentation(
                    contig=p.contig, pos=p.position + 1, ref=p.deleted, alt=p.inserted
                )
                if p.deleted and p.inserted
                else None,
                identifiers=[text],
            )
            for p in placements
        ...
        result.status = "ambiguous"
        col.warnings.append(
            f"{text} is multi-allelic ({len(placements)} alternate alleles); specify the allele"
        )
        return None
    ...
    async def _protein_candidates(
        ...
        unique = {(p[1], p[2], p[3], p[4]): p for p in placements}
        result.candidates = [
            self._placement_candidate(p, assembly, "ensembl") for p in unique.values()
        ]
        if not unique:
    ...
    async def lookup_variant(
        ...
        *,
        ...
        result = LookupVariantResult(
            tool="lookup_variant",
            status=norm.status,
            query=_query(request),
            normalization=norm,
            canonical_variant=norm.canonical_variant,
        )
        ...
        if variant is None or norm.status in ("error", "ambiguous", "unresolved"):
            if norm.status in ("ok", "partial"):
                result.status = "error"
            col.warnings.append(
                "No source lookups were made because the input did not resolve to one allele."
            )
            self._finish_norm_copy(norm, col, n_norm_errors)
            self._finish(result, col)
            return result
        ...
        for source in sources:
            ...
            if source == "ensembl":
                tasks[source] = _guard(
                    "ensembl",
                    "vep/region",
                    self.ensembl.vep(variant, deadline=deadline),
                    col,
                    budget_s=self._remaining(deadline, "ensembl"),
                )
            elif source == "clinvar":
                tasks[source] = _guard(
                    "clinvar",
                    "lookup",
                    self._clinvar(variant, request.max_clinvar_records, deadline, col),
                    col,
                    budget_s=self._remaining(deadline, "clinvar"),
                )
            elif source == "gnomad":
                tasks[source] = _guard(
                    "gnomad",
                    "variant",
                    self.gnomad.variant(variant, dataset=request.gnomad_dataset, deadline=deadline),
                    col,
                    budget_s=self._remaining(deadline, "gnomad"),
                )
            elif source == "alphagenome_atlas":
                tasks[source] = _guard(
                    "alphagenome_atlas",
                    "get_dense_variant_scores",
                    self._atlas_scores(variant, request.atlas_scorers, deadline),
                    col,
                    budget_s=self._remaining(deadline, "alphagenome_atlas"),
                )
        ...
        if not any(
            (
                ...
                evidence.clinical,
                evidence.population,
                evidence.functional_prediction,
            )
        ):
    ...
    async def _atlas_scores(
        self, variant: CanonicalVariant, scorers: list[str], deadline: float
    ) -> Evidence:
        return await self.atlas().variant_scores(
            variant, requested_scorers=scorers, deadline=deadline
        )

    async def _clinvar(
        self, variant: CanonicalVariant, max_records: int, deadline: float, col: _Collector
    ) -> list[Evidence]:
        ...
        summary = Evidence(
            ...
            limitations=match.notes
            + (
                []
                if match.matched_ids
                else [
        ...
        if variant.normalization_status != "reference_normalized" and variant.variant_class not in (
            ...
            summary.limitations.append(
                "Indel was not reference-normalized; ClinVar matching may miss equivalent records."
            )
        out = [summary]
        ids = match.matched_ids[:max_records]
        if len(match.matched_ids) > max_records:
            summary.truncation.append(
                Truncation(
                    field="matched records fetched",
                    returned=max_records,
                    available=len(match.matched_ids),
                    reason="max_clinvar_records",
                )
            )
        if ids:
    ...
    async def lookup_gene(
        ...
        *,
        ...
        resolution, ensg, symbol = await self._resolve_gene(
            request.gene, request.id_type, request.assembly, deadline, col
        )
        ...
        if "transcripts" in include and self._enabled("ensembl"):
            tasks["ensembl"] = _guard(
                "ensembl",
                "lookup/id",
                self._ensembl_gene(ensg, request.assembly, deadline),
                col,
                budget_s=self._remaining(deadline, "ensembl"),
            )
        if "protein" in include and self._enabled("uniprot"):
            tasks["uniprot"] = _guard(
                "uniprot",
                "entry",
                self._gene_proteins(record, symbol, deadline, col),
                col,
                budget_s=self._remaining(deadline, "uniprot"),
            )
        if "disease" in include and self._enabled("open_targets"):
            tasks["open_targets"] = _guard(
                "open_targets",
                "target_associations",
                self.opentargets.associations(
                    ensg, size=request.max_associations, deadline=deadline
                ),
                col,
                budget_s=self._remaining(deadline, "open_targets"),
            )
        if "constraint" in include and self._enabled("gnomad"):
            tasks["gnomad"] = _guard(
                "gnomad",
                "gene_constraint",
                self.gnomad.constraint(ensg, request.assembly, deadline=deadline),
                col,
                budget_s=self._remaining(deadline, "gnomad"),
            )
    ...
    async def _ensembl_gene(self, ensg: str, assembly: Assembly, deadline: float) -> Evidence:
        ...
        return await self.ensembl.entity_evidence(
            record, assembly, requested_id=ensg, deadline=deadline
        )

    async def _gene_proteins(
        self, record: dict[str, Any], symbol: str | None, deadline: float, col: _Collector
    ) -> list[Evidence]:
        ids = [i for i in record.get("uniprot_ids") or [] if isinstance(i, str)]
        if not ids and symbol:
            search = await self.uniprot.search(
                f"gene_exact:{symbol} AND organism_id:9606 AND reviewed:true", deadline=deadline
            )
            ids = search.reviewed
            if ids:
                col.warnings.append(
                    "HGNC lists no UniProt accession; reviewed UniProt entries found by gene name search"
                )
        if len(ids) > 3:
            col.warnings.append(
                f"{len(ids)} UniProt accessions linked; only the first 3 entries are returned"
            )
        out = []
        for acc in ids[:3]:
    ...
    async def _resolve_gene(
        ...
        if resolution is not None:
            ...
            if resolution.status == "resolved" and resolution.record:
                return (
                    resolution,
                    resolution.record.get("ensembl_gene_id"),
                    resolution.record.get("symbol"),
                )
            if resolution.status == "ambiguous":
    ...
    async def lookup_protein(
        ...
        *,
        ...
        deadline = self._deadline(deadline_s)
        col = _Collector(reference_provider=reference_provider or self.reference_provider)
        result = LookupProteinResult(
            tool="lookup_protein", status="error", query=request.model_dump()
        )
        ...
        if ACCESSION_RE.match(upper) and not upper.isalpha():
            accession = upper
        elif ENSEMBL_ID_RE.match(upper) and upper.startswith(("ENSP", "ENST")):
            ...
            if not accession_has_version(upper):
                col.warnings.append(
                    f"{query} has no version; UniProt cross-references are versioned"
                )
            search_query = f"xref:refseq-{upper}"
        else:
            resolution, _, symbol = await self._resolve_gene(
                query, None, request.assembly, deadline, col
            )
        ...
        if accession is None and search_query is not None:
            ...
            if len(search.reviewed) == 1:
                ...
                col.transformations.append(
                    Transformation(
                        operation="select_reviewed_entry",
                        source="uniprot",
                ...
                if search.candidates:
                    col.warnings.append(
                        f"UniProt query {search_query!r} matched {len(search.candidates)} entries "
        ...
        if "uniprot" not in col.sources:
            col.sources.append("uniprot")
        ev = self.uniprot.entry_evidence(
            entry, release, steps, include_all_features=request.include_all_features
        )
        if requested_version:
            stable, version = requested_version
            linked = [
                x
                for x in _flatten_xrefs(ev.data.get("cross_references", {}).get("Ensembl", []))
                if x.split(".")[0] == stable
            ]
            if linked and all(x != f"{stable}.{version}" for x in linked):
                ev.limitations.append(
                    f"UniProt links {', '.join(linked)}, not the requested {stable}.{version}."
                )
        result.evidence.append(ev)
        result.protein = {
            k: ev.data.get(k)
            for k in ("accession", "uniprotkb_id", "reviewed", "recommended_name", "genes")
        }
        result.status = "partial" if col.errors else "ok"
        self._finish(result, col)
        return result
        ...
        *,
        ...
        if kind != "unknown":
            norm = await self._normalize(
                NormalizeVariantRequest(variant=text, assembly=request.assembly), deadline, col
            )
            result = ResolveIdentifierResult(
                tool="resolve_identifier",
                status=norm.status,
                query=request.model_dump(),
                identifier_type=f"variant:{kind}",
                variant_candidates=norm.candidates,
            )
        ...
        if (
            REFSEQ_TX_RE.match(upper)
            or REFSEQ_PROTEIN_RE.match(upper)
            or re.match(r"^(NM_|NR_)\d+$", upper)
        ):
        ...
        if (id_type or detected) == "uniprot_ids":
            try:
                ...
                result.evidence.append(ev)
                result.resolved["uniprot"] = {
                    k: ev.data.get(k) for k in ("accession", "reviewed", "uniprotkb_id", "genes")
                }
    ...
    async def _resolve_vcv(
        self, m: re.Match[str], request: ResolveIdentifierRequest, deadline: float, col: _Collector
    ) -> ResolveIdentifierResult:
        result = ResolveIdentifierResult(
            tool="resolve_identifier",
            status="error",
        ...
        if (
            m.group(2)
            and record.source_record_version
            and m.group(2) != record.source_record_version
        ):
            col.warnings.append(
                f"Requested VCV version {m.group(2)}; ClinVar returned current version {record.source_record_version}"
            )
        ...
        result.resolved = {
            "clinvar": {
                "variation_id": record.data.get("variation_id"),
                "vcv": record.data.get("vcv"),
                "name": record.data.get("name"),
                "canonical_spdi": record.data.get("canonical_spdi"),
            },
            "locations": locations,
        }
        col.limitations.append(
            "Locations are as reported by ClinVar (1-based VCF fields); run normalize_variant on "
    ...
    async def _resolve_ensembl_feature(
        self, identifier: str, request: ResolveIdentifierRequest, deadline: float, col: _Collector
    ) -> ResolveIdentifierResult:
        result = ResolveIdentifierResult(
            tool="resolve_identifier",
            status="error",
            query=request.model_dump(),
            identifier_type="ensembl_transcript"
            if identifier.startswith("ENST")
            else "ensembl_protein",
        )
        assembly: Assembly = "GRCh38"
        if request.assembly:
            try:
        ...
        if record is not None:
            col.sources.append("ensembl")
            ev = await self.ensembl.entity_evidence(
                record, assembly, requested_id=identifier, deadline=deadline
            )
            result.evidence.append(ev)
            result.resolved["ensembl"] = {
                k: ev.data.get(k) for k in ("object_type", "id", "Parent", "biotype", "assembly")
            }
            ...
            if identifier.startswith("ENSP") and parent:
                try:
                    ...
                    parent = None
            if parent:
                resolution, _, _ = await self._resolve_gene(
                    parent, "ensembl_gene_id", assembly, deadline, col
                )
                if resolution and resolution.record:
                    result.resolved["gene"] = {
                        k: resolution.record.get(k)
                        for k in ("hgnc_id", "symbol", "ensembl_gene_id")
                    }
                    result.evidence.extend(resolution.evidence)
        ...
        try:
            ...
            col.error(exc.error)
        result.status = (
            ("partial" if col.errors else "ok")
            if result.resolved
            else ("error" if col.errors else "unresolved")
        )
        self._finish(result, col)
        return result

    async def _resolve_refseq(
        self, identifier: str, request: ResolveIdentifierRequest, deadline: float, col: _Collector
    ) -> ResolveIdentifierResult:
        ...
        tasks: list[Awaitable[Any]] = [
            _guard(
                "uniprot",
                "search",
                self.uniprot.search(f"xref:refseq-{identifier}", deadline=deadline),
                col,
                budget_s=self._remaining(deadline, "uniprot"),
            )
        ]
        if not protein:
            ...
            tasks.append(
                _guard(
                    "hgnc",
                    "fetch/refseq_accession",
                    self.hgnc.fetch("refseq_accession", unversioned, deadline),
                    col,
                    budget_s=self._remaining(deadline, "hgnc"),
                )
            )
        ...
        if not protein and values[1] is not None:
            ...
            if len(docs) == 1:
                result.resolved["gene"] = {
                    k: docs[0].get(k)
                    for k in ("hgnc_id", "symbol", "ensembl_gene_id", "mane_select")
                }
                if identifier not in (docs[0].get("mane_select") or []):
                    col.warnings.append(
                        "HGNC links this RefSeq accession without version; the gene mapping is not version-specific"
                    )
            elif docs:
...
def source_hosts(source: str) -> frozenset[str]:
    """Hosts of the documented public endpoints for a source; nothing else is contacted."""
    info = SOURCES[source]
    return frozenset(h for h in (urlsplit(u).hostname for u in info.base_urls) if h)
...
def _query(request: Any) -> dict[str, Any]:
    ...
    if (
        isinstance(ref, dict)
        and isinstance(ref.get("sequence"), str)
        and len(ref["sequence"]) > 200
    ):
...
def _enforce_size(result: ToolResult) -> None:
    ...
    if isinstance(ev, list):
        ...
        lists.extend(
            (f"evidence.{k}", getattr(ev, k))
            for k in ("clinical", "consequence", "population", "functional_prediction")
        )
