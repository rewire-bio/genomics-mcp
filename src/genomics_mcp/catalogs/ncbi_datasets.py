...
from genomics_mcp.archives._common.workspace import artifact_path, open_temp
...
def _asm(acc: str) -> str:
    a = acc.strip()
    if not _ASM.match(a):
        raise InvalidInputError(f"not a GenBank/RefSeq assembly accession: {acc!r}", source=SOURCE,
...
class NcbiDatasetsClient:
    def __init__(self, client: httpx.AsyncClient, *, api_key: SecretStr | None = None,
        ...
        self.http = SourceHttp(client, SourcePolicy(
            SOURCE, HOSTS, min_interval_s=0.11 if api_key else 0.21, timeout_s=timeout_s,
            terms_url=TERMS_URL))

    def _headers(self) -> dict[str, str]:
        return {"api-key": self.api_key.get_secret_value()} if self.api_key else {}

    async def _get(self, path: str, params: dict[str, Any] | None = None) -> tuple[dict, Provenance]:
        url = f"{BASE}{path}"
        data, res = await self.http.get_json(url, params=params, headers=self._headers())
        if not isinstance(data, dict) or not data:
            raise NotFoundError(f"NCBI Datasets has no record for {path}", source=SOURCE,
                                details={"http_status": res.status, "empty_body": True})
        version = res.headers.get("x-datasets-version")
        prov = Provenance(source=SOURCE, url=res.url, method="NCBI Datasets v2 REST", terms_url=TERMS_URL,
    ...
    def _dataset(self, r: dict, prov: Provenance) -> Dataset:
        ...
        if r.get("paired_accession"):
            links.append(EntityLink(relation="paired_assembly", kind=EntityKind.REFERENCE,
                                    accession=r["paired_accession"], source=SOURCE))
        bs = (info.get("biosample") or {}).get("accession")
        if bs:
            links.append(EntityLink(relation="biosample", kind=EntityKind.SAMPLE, accession=bs, source=SOURCE))
        if info.get("bioproject_accession"):
            links.append(EntityLink(relation="bioproject", kind=EntityKind.STUDY,
                                    accession=info["bioproject_accession"], source=SOURCE))
        p = prov.model_copy(update={"source_record_id": r.get("accession")})
        native = {k: r.get(k) for k in ("accession", "current_accession", "paired_accession", "source_database",
                                        "organism", "assembly_stats", "annotation_info")}
        native["assembly_info"] = {k: v for k, v in info.items() if k != "biosample"}
        return Dataset(
            accession=r["accession"], source=SOURCE, title=info.get("assembly_name"),
            description=info.get("description"), access_status=AccessStatus.OPEN,
            assemblies=[r["accession"]] + ([info["assembly_name"]] if info.get("assembly_name") else []),
            links=links, native=native, provenance=[p],
        )

    def _biosample(self, b: dict, prov: Provenance) -> Sample:
        desc = b.get("description") or {}
        org = desc.get("organism") or {}
        phen = [PhenotypeValue(name=a["name"], value=a.get("value"), source=SOURCE)
                for a in b.get("attributes") or [] if isinstance(a, dict) and a.get("name")]
        return Sample(
            accession=b["accession"], source=SOURCE, title=desc.get("title"), description=desc.get("comment"),
            organism=org.get("organism_name"), taxon_id=org.get("tax_id"), phenotypes=phen,
    ...
    async def search_datasets(self, query: str, *, limit: int | None = None,
        ...
        q = query.strip()
        if not q:
            raise InvalidInputError("NCBI Datasets search needs a taxon or assembly accession", source=SOURCE)
        if _ASM.match(q):
        ...
        return SourcePage(items=items, total=data.get("total_count"), provenance=[prov],
    ...
    async def describe_dataset(self, accession: str) -> DatasetDetail:
        ...
        ref = Reference(accession=r["accession"], source=SOURCE, assembly=r["accession"],
                        title=info.get("assembly_name"), description=info.get("description"),
                        links=list(ds.links), provenance=ds.provenance)
        warnings = []
        if r["accession"] != acc:
        ...
        related = {
            ...
            "ensembl_linkage": ("Ensembl identifies assemblies by INSDC accession; match "
                                f"{insdc} exactly (no name-based matching)") if insdc else None,
        }
    ...
    async def list_files(self, accession: str, *, limit: int | None = None,
        ...
        acc = _asm(accession)
        data, prov = await self._get(f"/genome/accession/{acc}/download_summary",
        ...
        for key, info in (data.get("available_files") or {}).items():
            ...
            reason = ("ZIP package; prepare_genome_fasta downloads, verifies and extracts the FASTA and "
                      "builds a .fai" if ann == "GENOME_FASTA" else
                      "ZIP package; extract locally before use (not prepared by this client)")
            items.append(FileRef(
                uri=url, format=FileFormat.OTHER, source=SOURCE, accession=acc,
                access_status=AccessStatus.OPEN, visibility=Visibility.PUBLIC,
                relationships=[EntityLink(relation="package_of", kind=EntityKind.REFERENCE, accession=acc,
                                          source=SOURCE)],
                readiness=Readiness(state=ReadinessState.DOWNLOAD_REQUIRED, reasons=[reason]),
                native={"container": "zip", "content": key, "content_format": fmt.value,
                        "annotation_type": ann, "file_count": info.get("file_count"),
    ...
    async def list_samples(self, accession: str, *, limit: int | None = None,
        ...
        if not bs or not bs.get("accession"):
            return SourcePage(items=[], total=0, provenance=[prov],
                              warnings=["the assembly report names no BioSample"])
        sample = self._biosample(bs, prov)
        sample.links.append(EntityLink(relation="sample_of_assembly", kind=EntityKind.REFERENCE,
                                       accession=reports[0]["accession"], source=SOURCE))
    ...
    async def prepare_genome_fasta(self, accession: str, *, workspace: Path,
                                   budget_bytes: int = DEFAULT_PACKAGE_BUDGET) -> Artifact:
        ...
        acc = _asm(accession)
        if "." not in acc:
            raise InvalidInputError("preparation needs a versioned assembly accession", source=SOURCE)
        if budget_bytes <= 0:
            raise InvalidInputError("budget_bytes must be positive", source=SOURCE)
        summary, _ = await self._get(f"/genome/accession/{acc}/download_summary",
        ...
        if isinstance(est, int | float) and est * 1024 * 1024 > budget_bytes:
            raise BudgetExceededError(f"genome FASTA is about {est:g} MB, over the {budget_bytes}-byte budget",
                                      source=SOURCE, hint="Pass an explicit larger budget.")
        url = f"{BASE}/genome/accession/{acc}/download"
        zpath = artifact_path(workspace, f"{acc}.genome_fasta.zip")
        dl = await self.http.download(url, zpath, budget_bytes=budget_bytes, headers=self._headers(),
                                      params={"include_annotation_type": "GENOME_FASTA"},
                                      timeout_s=self.transfer_timeout_s)
        dest = artifact_path(workspace, f"{acc}.fna")
        try:
            members, verified = await asyncio.to_thread(_extract_fasta, zpath, acc, dest, budget_bytes)
        finally:
            zpath.unlink(missing_ok=True)
        await asyncio.to_thread(pysam.faidx, str(dest))
        md5, sha = file_digests(dest)
        origin = FileRef(uri=f"{url}?include_annotation_type=GENOME_FASTA", format=FileFormat.OTHER,
                         source=SOURCE, accession=acc, access_status=AccessStatus.OPEN,
                         visibility=Visibility.PUBLIC, size_bytes=dl.size_bytes,
                         checksums=[Checksum(algorithm="sha256", value=dl.sha256)],
                         native={"container": "zip", "members": members})
        prov = Provenance(
            source=SOURCE, source_record_id=acc, url=f"{url}?include_annotation_type=GENOME_FASTA",
            method="NCBI Datasets v2 genome package", terms_url=TERMS_URL,
            transformations=[f"downloaded {dl.size_bytes}-byte ZIP within budget",
                             f"extracted {len(members)} FASTA member(s) in name order",
                             "member MD5s verified against md5sum.txt" if verified else
                             "package had no md5sum.txt entries for the FASTA members",
                             "ZIP removed", "indexed locally with pysam.faidx"],
        )
        return Artifact(path=str(dest), size_bytes=dest.stat().st_size,
                        checksums=[Checksum(algorithm="md5", value=md5), Checksum(algorithm="sha256", value=sha)],
                        checksum_verified=verified, format=FileFormat.FASTA, index_path=f"{dest}.fai",
                        origin=origin, provenance=prov)


def _extract_fasta(zpath: Path, acc: str, dest: Path, budget: int) -> tuple[list[str], bool]:
    ...
    with zf:
        ...
        if declared > budget:
            raise BudgetExceededError(f"extracted FASTA would be {declared} bytes, over the budget",
                                      source=SOURCE, details={"budget_bytes": budget})
        sums: dict[str, str] = {}
        if "md5sum.txt" in zf.namelist():
            for line in zf.read("md5sum.txt").decode("utf-8", "replace").splitlines():
                parts = line.split()
                if len(parts) == 2:
                ...
                for n in names:
                    ...
                    with zf.open(n) as src:
                        for chunk in iter(lambda: src.read(1 << 20), b""):
                            written += len(chunk)
                            if written > budget:
                                raise BudgetExceededError("extracted FASTA exceeded the budget", source=SOURCE)
                            h.update(chunk)
                            out.write(chunk)
