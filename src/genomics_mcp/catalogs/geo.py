...
def geo_kind(acc: str) -> str:
    ...
    if not m:
        raise InvalidInputError(f"not a GEO accession: {acc!r}", source=SOURCE,
...
class GeoClient:
    def __init__(self, client: httpx.AsyncClient, *, api_key: SecretStr | None = None,
                 timeout_s: float = DEFAULT_OPERATION_TIMEOUT_S) -> None:
        self.source = SOURCE
    ...
    async def _soft(self, acc: str) -> tuple[dict[str, list[str]], Provenance]:
        ...
        if not text.lstrip().startswith("^"):
            ...
            raise UpstreamError("GEO returned a non-SOFT response", source=SOURCE,
        ...
        prov = Provenance(source=SOURCE, source_record_id=acc, url=res.url, method="GEO acc.cgi SOFT (brief)",
                          terms_url=TERMS_URL)
        return match[2], prov
    ...
    def _file(self, url: str, owner_kind: EntityKind, owner: str) -> FileRef:
        ...
        if not https:
            ...
            readiness = Readiness(state=ReadinessState.DOWNLOAD_REQUIRED,
                                  reasons=["tar bundle of the per-sample supplementary files, which are "
                                           "listed individually"])
        ...
        elif fmt in (FileFormat.BAM, FileFormat.CRAM, FileFormat.VCF, FileFormat.BED, FileFormat.GFF3,
                     FileFormat.GTF):
        ...
        return FileRef(
            uri=uri, format=fmt or FileFormat.OTHER, compression=compression_from_name(name), source=SOURCE,
            accession=owner, access_status=AccessStatus.OPEN, visibility=Visibility.PUBLIC,
            relationships=[EntityLink(relation="supplementary_file_of", kind=owner_kind, accession=owner,
                                      source=SOURCE)],
            readiness=readiness, native={"geo_url": url, "file_name": name},
        )
    ...
    def _sample(self, acc: str, a: dict[str, list[str]], prov: Provenance) -> Sample:
        ...
        for ch in range(1, channels + 1):
            for line in a.get(f"Sample_characteristics_ch{ch}", []):
                ...
                if channels > 1:
                    name = f"{name} (ch{ch})"
                phen.append(PhenotypeValue(name=name, value=value.strip() if sep else line, source=SOURCE))
        if channels > 1:
            prov.transformations.append("characteristic names suffixed with their channel (chN)")
        links = [EntityLink(relation="part_of", kind=EntityKind.STUDY, accession=s, source=SOURCE)
        ...
        for rel in a.get("Sample_relation", []):
            ...
            if kind.strip() == "BioSample" and tail:
                links.append(EntityLink(relation="biosample", kind=EntityKind.SAMPLE, accession=tail, source=SOURCE))
            elif kind.strip() == "SRA" and tail:
                links.append(EntityLink(relation="sra_experiment", kind=EntityKind.EXPERIMENT, accession=tail,
                                        source=SOURCE))
        taxid = _first(a, "Sample_taxid_ch1")
        files = [self._file(u, EntityKind.SAMPLE, acc) for k, v in a.items()
                 if k.startswith("Sample_supplementary_file") for u in v if u and u.upper() != "NONE"]
        native = _public(a)
        native["supplementary_files"] = [f.uri for f in files]
        native["platforms"] = a.get("Sample_platform_id", [])
        return Sample(
            accession=acc, source=SOURCE, title=_first(a, "Sample_title"),
            description=_first(a, "Sample_description"), organism=_first(a, "Sample_organism_ch1"),
            taxon_id=int(taxid) if taxid and taxid.isdigit() else None, phenotypes=phen, links=links,
    ...
    async def search_datasets(self, query: str, *, limit: int | None = None,
        ...
        data, res = await self.http.get_json(f"{EUTILS}/esearch.fcgi",
                                             params=self._eparams(term=term, retstart=off, retmax=n))
        ...
        ids = r.get("idlist", [])
        prov = Provenance(source=SOURCE, url=res.url, method="NCBI E-utilities esearch db=gds", terms_url=TERMS_URL)
        items: list[Dataset] = []
        if ids:
            summ, sres = await self.http.get_json(f"{EUTILS}/esummary.fcgi",
                                                  params=self._eparams(id=",".join(ids)))
            result = (summ or {}).get("result", {})
            sprov = Provenance(source=SOURCE, url=sres.url, method="NCBI E-utilities esummary db=gds",
                               terms_url=TERMS_URL)
            for uid in result.get("uids", []):
                ...
                items.append(Dataset(
                    accession=acc, source=SOURCE, title=s.get("title"), description=s.get("summary"),
                    access_status=AccessStatus.OPEN,
                    native={k: s.get(k) for k in ("entrytype", "gdstype", "taxon", "n_samples", "gpl",
                                                   "suppfile", "pdat", "bioproject", "pubmedids")},
                    provenance=[sprov],
                ))
    ...
    async def describe_dataset(self, accession: str) -> DatasetDetail:
        ...
        kind = geo_kind(acc)
        if kind == "GSM":
            raise InvalidInputError(f"{acc} is a GEO sample; use get_sample_metadata", source=SOURCE)
        if kind == "GPL":
            raise InvalidInputError(f"{acc} is a GEO platform; use describe_platform", source=SOURCE)
        if kind == "GDS":
            raise UnsupportedError("GDS curated datasets are not described here; use the parent GSE",
                                   source=SOURCE)
        ...
        for rel in a.get("Series_relation", []):
            ...
            if kind_.strip() == "BioProject" and tail:
                links.append(EntityLink(relation="bioproject", kind=EntityKind.STUDY, accession=tail, source=SOURCE))
            elif kind_.strip() in ("SuperSeries of", "SubSeries of") and tail:
                links.append(EntityLink(relation=kind_.strip().lower().replace(" ", "_"), kind=EntityKind.STUDY,
                                        accession=tail, source=SOURCE))
        samples = a.get("Series_sample_id", [])
        ds = Dataset(
            accession=acc, source=SOURCE, title=_first(a, "Series_title"), description=_first(a, "Series_summary"),
            access_status=AccessStatus.OPEN, links=links, native=_public(a), provenance=[prov],
        )
        related = {"sample_count": len(samples), "series_supplementary_files":
    ...
    async def describe_platform(self, accession: str) -> dict[str, Any]:
        ...
        data, res = await self.http.get_json(f"{EUTILS}/esummary.fcgi", params=self._eparams(id=uid))
        ...
        native = {k: rec.get(k) for k in ("title", "taxon", "gdstype", "ptechtype", "entrytype", "pdat",
                                          "n_samples")}
        ...
        prov = Provenance(source=SOURCE, source_record_id=acc, url=res.url,
                          method="NCBI E-utilities esummary db=gds", terms_url=TERMS_URL)
    ...
    async def list_files(self, accession: str, *, limit: int | None = None,
        ...
        if kind == "GSM":
            ...
            return SourcePage(items=[self._file(u, EntityKind.SAMPLE, acc) for u in files],
                              total=len(files), provenance=[prov])
        if kind != "GSE":
        ...
        provs = [prov]
        if off == 0:
            items += [self._file(u, EntityKind.STUDY, acc) for u in a.get("Series_supplementary_file", [])
                      if u.upper() != "NONE"]
        page = sample_ids[off:off + n]
        for gsm in page:
            sa, sprov = await self._soft(gsm)
            provs.append(sprov)
            items += [self._file(u, EntityKind.SAMPLE, gsm) for k, v in sa.items()
                      if k.startswith("Sample_supplementary_file") for u in v if u and u.upper() != "NONE"]
        return SourcePage(items=items, provenance=provs,
                          next_cursor=next_offset_cursor(SOURCE, scope, off, len(page), len(sample_ids)),
                          warnings=[f"paged by sample ({len(sample_ids)} samples); series-level files "
    ...
    async def list_samples(self, accession: str, *, limit: int | None = None,
        ...
        return SourcePage(items=items, total=len(sample_ids), provenance=[prov],
                          next_cursor=next_offset_cursor(SOURCE, scope, off, len(page), len(sample_ids)))
