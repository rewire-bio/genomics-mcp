...
def host_is_blocked(host: str) -> bool:
    ...
    try:
        ...
        return False
    return ip.is_link_local or ip.is_loopback or ip.is_private or ip.is_unspecified or ip.is_multicast
...
def status_error(source: str, status: int, url: str, body: bytes = b"") -> SourceError:
    ...
    if status == 403:
        return UnauthorizedError(
            f"{source}: permission denied", source=source, details=details,
    ...
    if status in (400, 422):
        return InvalidInputError(f"{source}: request rejected: {msg or status}", source=source, details=details)
    if status == 429:
...
class SourceHttp:
    def __init__(
        ...
        *,
        ...
        self.client = client
        self.policy = policy
        self.limiter = RateLimiter(policy.min_interval_s, clock)
        self._clock = clock
    ...
    def check_url(self, url: str, *, allowed_hosts: frozenset[str] | None = None) -> None:
        ...
        if p.scheme != "https":
            raise InvalidInputError(
                f"{self.policy.name}: only https URLs are fetched", source=self.policy.name,
    ...
    async def request(
        ...
        *,
        ...
        if max_body is not None and max_body <= 0:
            raise BudgetExceededError(f"{self.policy.name}: no byte budget left for this request",
                                      source=self.policy.name, details={"url": redact_url(url)})
        timeout_s = timeout_s or self.policy.timeout_s
        deadline = self._clock() + timeout_s
        try:
            async with asyncio.timeout(timeout_s):
                return await self._request(
                    method, url, params=params, headers=headers, data=data, ok=ok,
                    max_body=self.policy.max_body_bytes if max_body is None else max_body, deadline=deadline,
    ...
    async def _request(self, method, url, *, params, headers, data, ok, max_body, deadline, allowed_hosts):
        attempt = 0
        while True:
            try:
                result = await self._send_following(
                    method, url, params=params, headers=headers, data=data, max_body=max_body,
                    allowed_hosts=allowed_hosts,
                )
            except httpx.TimeoutException as exc:
                raise DeadlineExceededError(
                    f"{self.policy.name}: request timed out", source=self.policy.name,
                    details={"url": redact_url(url)},
                ) from exc
            except httpx.TransportError as exc:
                if attempt < self.policy.max_retries:
                    attempt += 1
                    await asyncio.sleep(min(2.0 ** attempt * 0.25, max(0.0, deadline - self._clock())))
                    continue
                raise UpstreamError(
    ...
    async def _send_following(self, method, url, *, params, headers, data, max_body, allowed_hosts):
        ...
        for _ in range(MAX_REDIRECTS + 1):
            ...
            if _origin(current) != origin:
                hdrs = {k: v for k, v in hdrs.items() if k.lower() not in ("authorization", "cookie")}
    ...
    async def get_json(self, url: str, **kw: Any) -> tuple[Any, HttpResult]:
        headers = {"Accept": "application/json", **(kw.pop("headers", None) or {})}  # caller may override
    ...
    async def resolve_redirect(self, url: str, *, allowed_hosts: frozenset[str] | None = None) -> str:
        """Return the final URL after allowlisted redirects (issues one GET with Range 0-0)."""
        res = await self.request("GET", url, headers={"Range": "bytes=0-0"}, ok=(200, 206),
                                 max_body=1024 * 1024, allowed_hosts=allowed_hosts)
        return res.url

    async def probe_range(self, url: str, *, nbytes: int = 64,
                          allowed_hosts: frozenset[str] | None = None) -> RangeProbe:
        """GET bytes=0..nbytes-1 and report whether the server honoured the range."""
        res = await self.request("GET", url, headers={"Range": f"bytes=0-{nbytes - 1}"}, ok=(200, 206),
                                 max_body=max(nbytes, 1024 * 1024), allowed_hosts=allowed_hosts)
        total = None
        ...
        return RangeProbe(
            url=res.url, status=res.status, range_supported=res.status == 206 and cr.startswith("bytes 0-"),
            total_size=total, head=res.body[:nbytes], etag=res.headers.get("etag"),
        )
        ...
        *,
        ...
        if expected_size is not None and expected_size > budget_bytes:
            raise BudgetExceededError(
                ...
                source=self.policy.name, hint="Pass an explicit larger transfer budget.",
        ...
        try:
            ...
            raise DeadlineExceededError(
                f"{self.policy.name}: download exceeded {timeout_s:g}s", source=self.policy.name,
                details={"url": redact_url(url)},
            ) from exc
    ...
    async def _download(self, url, dest, budget, headers, params, allowed_hosts) -> Download:
        ...
        for _ in range(MAX_REDIRECTS + 1):
            ...
            if _origin(current) != origin:
                hdrs = {k: v for k, v in hdrs.items() if k.lower() not in ("authorization", "cookie")}
            await self.limiter.wait()
            req = self.client.build_request("GET", current, params=params if current == url else None,
                                            headers=hdrs)
            resp = await self.client.send(req, stream=True, follow_redirects=False)
            try:
                if resp.status_code in (301, 302, 303, 307, 308) and "location" in resp.headers:
                ...
                if resp.status_code != 200 and not _is_whole_206(resp):
                    body = await _read_bounded(resp, 64 * 1024, self.policy.name, str(resp.url), strict=False)
                ...
                if declared and declared.isdigit() and int(declared) > budget:
                    raise BudgetExceededError(
                        ...
                        source=self.policy.name, details={"budget_bytes": budget},
                    )
...
async def _read_bounded(resp: httpx.Response, limit: int, source: str, url: str, *, strict: bool = True) -> bytes:
    chunks: list[bytes] = []
    total = 0
    async for chunk in resp.aiter_bytes():  # decoded bytes: bounds decompressed size too
        ...
        if total > limit:
            if not strict:
                ...
                break
            raise BudgetExceededError(
                f"{source}: response exceeded {limit} bytes", source=source,
...
async def _stream_to(resp: httpx.Response, dest: Path, budget: int, source: str, url: str) -> Download:
    ...
    try:
        with os.fdopen(fd, "wb") as fh:
            async for chunk in resp.aiter_bytes():
                ...
                if size > budget:
                    raise BudgetExceededError(
                        f"{source}: download exceeded the {budget}-byte budget", source=source,
