...
_REGION_FORMATS = {
    FileFormat.BAM: "reads",
    FileFormat.CRAM: "reads",
    FileFormat.VCF: "variants",
...
def _require(acc: str, kind: str) -> str:
    ...
    if got != kind:
        raise InvalidInputError(
            f"{acc} is an EGA {got} accession; this operation needs a {kind}", source=SOURCE
        )
    return acc
...
class EgaClient:
    ...
    async def _list(
        self, path: str, limit: int, offset: int
    ) -> tuple[list[dict], int | None, Provenance]:
        url = f"{self.metadata_base}{path}"
        data, res = await self.meta.get_json(
            url, params={"limit": limit, "offset": offset}, ok=(200, 206)
        )
        if data is None:
            data = []
    ...
    async def _bearer(self) -> str:
        if self.auth is None:
            raise UnauthorizedError(
                "EGA file access needs explicit credentials",
                source=SOURCE,
    ...
    async def _authed_json(self, path: str, params: dict | None = None) -> tuple[Any, Any]:
        ...
        return await self.data.get_json(
            f"{self.data_base}{path}", params=params, headers={"Authorization": f"Bearer {bearer}"}
        )
        ...
        if d.get("policy_accession_id"):
            links.append(
                EntityLink(
                    ...
                    accession=d["policy_accession_id"],
                    source=SOURCE,
                )
            )
        return Dataset(
            ...
            access_status=_access(d.get("access_type")),
            policy_accession=d.get("policy_accession_id"),
            links=links,
            native=d,
            provenance=[prov],
        )
    ...
    def _sample(self, s: dict, prov: Provenance, links: list[EntityLink] | None = None) -> Sample:
        phen = [
            PhenotypeValue(name=k, value=None if s.get(k) is None else str(s[k]), source=SOURCE)
            for k in ("phenotype", "biological_sex")
            if k in s
        ]
        return Sample(
            accession=s["accession_id"],
    ...
    def _public_file(self, f: dict, dataset: Dataset | None, prov: Provenance) -> FileRef:
        ...
        if ds_acc:
            rel.append(
                EntityLink(
                    relation="part_of", kind=EntityKind.DATASET, accession=ds_acc, source=SOURCE
                )
            )
        access = dataset.access_status if dataset else AccessStatus.UNKNOWN
        checks = []
        if (
            f.get("unencrypted_checksum")
            and str(f.get("unencrypted_checksum_type", "")).upper() == "MD5"
        ):
            checks.append(Checksum(algorithm="md5", value=f["unencrypted_checksum"].lower()))
        native = {k: v for k, v in f.items()}
        native["size_note"] = (
            "archive filesize; the decrypted plain file size can differ, "
        ...
        return FileRef(
            ...
            checksums=checks,
            relationships=rel,
            readiness=self._readiness(fmt, ext, authed=False),
            native=native,
        )
    ...
    def _authed_file(self, f: dict, access: AccessStatus) -> FileRef:
        ...
        rel = [
            EntityLink(relation="part_of", kind=EntityKind.DATASET, accession=d, source=SOURCE)
            for d in f.get("datasetId") or []
        ]
        idx = f.get("indexFileId") or None
        if idx:
            rel.append(
                EntityLink(relation="has_index", kind=EntityKind.FILE, accession=idx, source=SOURCE)
            )
        checks = []
        ...
        return FileRef(
            uri=f"ega://{acc}",
            index_uri=f"ega://{idx}" if idx else None,
            format=fmt,
            source=SOURCE,
            accession=acc,
            ...
            checksums=checks,
            relationships=rel,
            readiness=self._readiness(fmt, ext, authed=True, has_index=bool(idx)),
            native=native,
        )

    @staticmethod
    def _readiness(
        fmt: FileFormat | None, ext: str, *, authed: bool, has_index: bool = False
    ) -> Readiness:
        ...
        if fmt in _REGION_FORMATS:
            why = (
                "EGA htsget access not yet verified for this file; call check_file"
                if authed
                else "controlled access: region queries need an authorised EGA htsget ticket"
            )
            reasons = [why]
        ...
        return Readiness(
            ...
            reasons=[
                "file format not reported by EGA public metadata"
                if not fmt
        ...
        if not q:
            ...
            if not q.upper().startswith("EGA"):
                raise UnsupportedError(
                    "the EGA public metadata API offers no free-text search",
                    source=SOURCE,
            ...
            path = {
                "study": f"/studies/{q}/datasets",
                "sample": f"/samples/{q}/datasets",
                "file": f"/files/{q}/datasets",
                "dac": f"/dacs/{q}/datasets",
            }.get(kind)
            if path is None:
                raise UnsupportedError(
                    f"EGA dataset search by {kind} accession is not offered", source=SOURCE
                )
            await self._one(
                path.rsplit("/", 1)[0]
            )  # distinguish unknown accession from no datasets
        scope = f"datasets:{q}"
    ...
    async def describe_dataset(self, accession: str) -> DatasetDetail:
        ...
        for s in studies:
            ds.links.append(
                EntityLink(
                    relation="part_of", kind=EntityKind.STUDY, accession=s.accession, source=SOURCE
                )
            )
        ...
        warnings = []
        if (
            related["sample_endpoint_count"] is not None
            and d.get("num_samples") is not None
            and related["sample_endpoint_count"] != d.get("num_samples")
        ):
            warnings.append(
                f"EGA reports num_samples={d.get('num_samples')} but its samples endpoint "
        ...
        return DatasetDetail(
            dataset=ds, studies=studies, related=related, provenance=provs, warnings=warnings
        )
    ...
    async def list_files(
        ...
        if self.auth is not None:
            ...
            data, res = await self._authed_json(
                f"/metadata/datasets/{acc}/files", params={"limit": want}
            )
            rows = data if isinstance(data, list) else []
            prov = Provenance(
                source=SOURCE,
                url=res.url,
                method="EGA download API metadata (authorised)",
                terms_url=TERMS_URL,
        ...
        return SourcePage(
            ...
            next_cursor=next_offset_cursor(SOURCE, scope, off, len(items), total),
            warnings=[
                "public EGA metadata has no file names or index links; configure EGA "
    ...
    async def list_samples(
        ...
        if kind not in ("dataset", "study", "file"):
            raise InvalidInputError(
                f"list_samples needs an EGA dataset, study or file, not a {kind}", source=SOURCE
            )
        n = page_size(limit)
        ...
        rel = {
            "dataset": ("part_of", EntityKind.DATASET),
            "study": ("part_of", EntityKind.STUDY),
            "file": ("sample_of_file", EntityKind.FILE),
        }[kind]
        items = [
            self._sample(
                r, prov, [EntityLink(relation=rel[0], kind=rel[1], accession=acc, source=SOURCE)]
            )
            for r in rows
        ]
        return SourcePage(
    ...
    async def get_sample_metadata(self, accession: str) -> Sample:
        ...
        for rel_path, kind in (("datasets", EntityKind.DATASET), ("studies", EntityKind.STUDY)):
            ...
            links += [
                EntityLink(
                    relation="part_of", kind=kind, accession=r["accession_id"], source=SOURCE
                )
                for r in rows
            ]
    ...
    async def check_file(self, accession: str, *, endpoint: str = "reads") -> FileRef:
        ...
        data, _ = await self.data.get_json(
            url, params={"format": fmt, "class": "header"}, headers=hg.ticket_headers(bearer)
        )
        ...
        ref.readiness = Readiness(
            ...
            checked_at=utcnow(),
            reasons=[
                f"EGA htsget {endpoint} ticket issued for {fmt} ({len(ticket.blocks)} header block(s))",
    ...
    async def get_region(
        ...
        *,
        ...
        if budget_bytes <= 0 or max_records < 0:
            raise InvalidInputError(
                "budget_bytes must be positive and max_records non-negative", source=SOURCE
            )
        fmt_u = hg.validate_region(
            interval, fmt or ("BAM" if endpoint == "reads" else "VCF"), endpoint, max_region_bp
        )
        ...
        params = {
            "referenceName": interval.contig,
            "start": interval.start,
            "end": interval.end,
            "format": fmt_u,
        }
        data, res = await self.data.get_json(
            url,
            params=params,
        ...
        ticket = hg.parse_ticket(data)
        payload, blocks = await hg.fetch_blocks(
            self.data,
            ticket,
            ticket_url=url,
            bearer=bearer,
            budget_bytes=budget_bytes,
            allowed_hosts=self.block_hosts,
        )
        if not payload:
        ...
        transformations = [
            ...
            "ticket MD5 verified"
            if ticket_md5
            else "ticket supplied no MD5; bytes not checksum-verified",
        ]
        prov = Provenance(
            source=SOURCE,
            source_record_id=acc,
            url=res.url,
            method=f"EGA htsget /{endpoint}",
            terms_url=TERMS_URL,
            transformations=list(transformations),
        )
        artifact = hg.artifact_for(
            path,
            len(payload),
            sha256,
            md5,
            fmt_u,
            hg.origin_file(acc, fmt_u),
            prov,
        ...
        if header.sq_assembly_tags and interval.assembly not in header.sq_assembly_tags:
            problems.append(
                f"caller assembly {interval.assembly!r} differs from header assembly tags "
        ...
        filt = [
            "overlap: reference_start < end and reference_end > start (reference_end from CIGAR)"
            if endpoint == "reads"
            else "overlap: record start < end and stop > start"
        ]
        if endpoint == "reads":
            filt.append("unmapped/unplaced records excluded; no flag or MAPQ filters applied")
        result_prov = prov.model_copy(
            update={
                "transformations": [
                    *transformations,
        ...
        return hg.RegionResult(
            ...
            records_in_blocks=parsed["total"],
            records_overlapping=parsed["overlapping"],
            records_skipped_unplaced=parsed["unplaced"],
            truncated=parsed["overlapping"] > len(parsed["records"]),
            filters=filt,
            warnings=problems,
    ...
    @operation()
    async def transfer_source(self, accession: str) -> dict[str, Any]:
        """What a transfer manager needs before streaming an authorised whole file.

        EGA v2 `fileSize` is the stored (encrypted) size; pyega3 (libs/data_file.py) downloads
        `destinationFormat=plain` as `fileSize - 16` bytes (the 16-byte IV is not sent). Both are
        reported; `plain_size_bytes` is derived only for that documented plain download."""
        await self._bearer()
        ref = await self.get_file(accession)
        stored = ref.native.get("fileSize")
        if not isinstance(stored, int) or stored <= 16:
            raise PreparationRequiredError(
                "EGA reports no usable stored size for this file", source=SOURCE
            )
        return {
            "file": ref,
            "stored_size_bytes": stored,
            "plain_size_bytes": stored - 16,
            "size_derivation": "pyega3 plain download: fileSize - 16 (IV)",
            "md5": next((c.value for c in ref.checksums if c.algorithm == "md5"), None),
            "name": ref.native.get("displayFileName"),
        }
    ...
    async def plain_stream(
        ...
        """Authorised bytes [start, end) of the decrypted file; the bearer stays on the EGA origin."""
        acc = _require(accession, "file")
        bearer = await self._bearer()
        async with self.data.open_range_stream(
            f"{self.data_base}/files/{acc}",
            start=start,
            end=end,
    ...
    async def fetch_file(
        ...
        if not isinstance(stored, int) or stored <= 16:
            raise PreparationRequiredError(
                "EGA reports no usable size for this file; refusing an unbounded download",
                source=SOURCE,
            )
        ...
        dest = artifact_path(
            workspace, str(ref.native.get("displayFileName") or f"{ref.accession}.bin")
        )
        dl = await self.data.download(
            f"{self.data_base}/files/{ref.accession}",
            dest,
            budget_bytes=budget_bytes,
            params={"destinationFormat": "plain"},
        ...
        if dl.size_bytes != plain:
            ...
            raise UpstreamError(
                "EGA download size differs from the expected plain size; file removed",
                source=SOURCE,
                details={"expected": plain, "observed": dl.size_bytes},
            )
        verified = md5 is not None and dl.md5 == md5
        if md5 is not None and not verified:
            dest.unlink(missing_ok=True)
            raise UpstreamError(
                "downloaded EGA file failed its MD5 check; file removed",
                source=SOURCE,
        ...
        prov = Provenance(
            ...
            method="EGA download API (destinationFormat=plain)",
            terms_url=TERMS_URL,
            transformations=[
                f"streamed bytes 0-{plain - 1} within budget",
