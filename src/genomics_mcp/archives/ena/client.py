...
from genomics_mcp.archives._common.workspace import artifact_path, remove_quietly
...
STUDY_FIELDS = (
    "study_accession,secondary_study_accession,study_title,study_description,tax_id,"
...
ANALYSIS_FIELDS = (
    "analysis_accession,study_accession,sample_accession,analysis_type,"
    "submitted_ftp,submitted_bytes,submitted_md5,submitted_format,generated_ftp,"
    "generated_bytes,generated_md5"
)
SAMPLE_FIELDS = (
    "sample_accession,secondary_sample_accession,sample_alias,sample_title,tax_id,"
...
def _sequence_digest(path: Path) -> tuple[str, int]:
    """MD5 of the upper-case sequence (ENA `sequence_md5` convention) and its length."""
...
class EnaClient:
    ...
    async def _portal(self, endpoint: str, params: dict[str, Any]) -> tuple[list[dict], Provenance]:
        ...
        data, _res = await self.http.get_json(url, params={**params, "format": "json"})
        if data is None:
            data = []
        if not isinstance(data, list):
            raise UpstreamError("ENA portal returned an unexpected payload", source=SOURCE)
        return data, self._prov(
            str(httpx.URL(url, params={**params, "format": "json"})),
            None,
    ...
    async def _count(self, endpoint: str, params: dict[str, Any]) -> int | None:
        data, _ = await self.http.get_json(
            f"{PORTAL}/{endpoint}", params={**params, "format": "json"}
        )
        try:
    ...
    async def _page_ids(
        self, endpoint: str, params: dict[str, Any], id_field: str, off: int, n: int
    ) -> tuple[list[str], int, Provenance]:
        """Sorted accession list (bounded), sliced to one page. Deterministic across calls."""
        rows, prov = await self._portal(
            endpoint, {**params, "fields": id_field, "limit": MAX_RECORDS + 1}
        )
        ...
        if len(rows) > MAX_RECORDS:
            raise InvalidInputError(
                ...
                "accession (sample or run)",
                source=SOURCE,
                details={"limit": MAX_RECORDS},
            )
    ...
    async def _details(self, result: str, ids: list[str], fields: str, id_field: str) -> list[dict]:
        ...
        if missing:
            raise UpstreamError(
                f"ENA returned no details for {len(missing)} listed accessions",
                source=SOURCE,
    ...
    async def _study_row(self, acc: str) -> tuple[dict, Provenance]:
        rows, prov = await self._portal(
            "search", {"result": "study", "includeAccessions": acc, "fields": STUDY_FIELDS}
        )
        if not rows:
    ...
    def _dataset(self, r: dict, prov: Provenance) -> Dataset:
        ...
        return Dataset(
            ...
            source=SOURCE,
            title=r.get("study_title") or None,
            description=r.get("study_description") or None,
    ...
    def _files_from_row(
        self, row: dict, owner_kind: EntityKind, owner: str, prov: Provenance
    ) -> list[FileRef]:
        ...
        for s in _split(row.get("sample_accession")):
            links.append(
                EntityLink(
                    relation="derived_from", kind=EntityKind.SAMPLE, accession=s, source=SOURCE
                )
            )
                ...
                entries.append(
                    {
                        ...
                        "is_index": is_index_name(name)
                        or (declared or "").upper() in _INDEX_FORMATS,
                    }
                )
        ...
        for i, e in enumerate(data):
            ...
            elif len(e["index_candidates"]) > 1 or (
                e["index_candidates"] and len(claims.get(e["index_candidates"][0], [])) > 1
            ):
        ...
        for x in indexes:
            ...
            native = {
                "file_group": x["group"],
                "declared_format": x["declared"],
                "https_available": x["https"],
                "role": "index",
                "note": "no unambiguous data file for this index in the same record",
            }
    ...
    def _ref(
        ...
        return FileRef(
            ...
            index_uri=index,
            format=fmt,
            compression=compression_from_name(e["name"]),
            source=SOURCE,
            accession=owner,
            access_status=AccessStatus.OPEN,
            ...
            readiness=self._readiness(fmt, index, e["https"], e["group"]),
            native=native,
        )
    ...
    def _readiness(fmt: FileFormat | None, index: str | None, https: bool, group: str) -> Readiness:
        ...
        if fmt in (FileFormat.BAM, FileFormat.CRAM, FileFormat.VCF, FileFormat.BCF):
            ...
            reasons.append(
                "index listed by ENA; range support and alignment state not yet verified "
                "(call check_file)"
            )
    ...
    async def search_datasets(
        ...
        if kind is not None:
            raise UnsupportedError(
                f"search_datasets takes a study accession or text; {q} is a {kind}",
                source=SOURCE,
                hint="Use get_sample_metadata or list_files.",
            )
        words = _words(q)
        if not words:
        ...
        ids, total, prov = await self._page_ids(
            "search", {"result": "study", "query": clause}, "study_accession", off, n
        )
    ...
    async def describe_dataset(self, accession: str) -> DatasetDetail:
        ...
        related = {
            "run_count": await self._count(
                "filereportcount", {"accession": primary, "result": "read_run"}
            ),
            "analysis_count": await self._count(
                "filereportcount", {"accession": primary, "result": "analysis"}
            ),
            "sample_count": await self._count(
                "count", {"result": "sample", "query": f"study_accession={primary}"}
            ),
        }
        warnings = []
        if primary != acc:
        ...
        return DatasetDetail(
            dataset=ds, studies=[study], related=related, provenance=[prov], warnings=warnings
        )
    ...
    async def list_files(
        ...
        results = (
            ["read_run", "analysis"]
            if kind in ("study", "sample")
            else (["analysis"] if kind == "analysis" else ["read_run"])
        )
        id_fields = {"read_run": "run_accession", "analysis": "analysis_accession"}
        all_ids: list[tuple[str, str]] = []
        provs: list[Provenance] = []
        for r in results:
            ids, _, prov = await self._page_ids(
                "filereport", {"accession": acc, "result": r}, id_fields[r], 0, MAX_RECORDS
            )
            provs.append(prov)
        ...
        return SourcePage(
            ...
            warnings=[
                f"paged by ENA run/analysis records ({total_rows} in total); "
    ...
    async def list_samples(
        ...
        ids, total, prov = await self._page_ids(
            "search", {"result": "sample", "query": query}, "sample_accession", off, n
        )
        ...
        for r in rows:
            items.append(
                Sample(
                    accession=r["sample_accession"],
                    source=SOURCE,
                    title=r.get("sample_title") or None,
    ...
    async def _browser_xml(self, acc: str) -> tuple[ET.Element, Provenance]:
        ...
        text, _res = await self.http.get_text(
            url, headers={"Accept": "application/xml"}, max_body=MAX_XML_BYTES
        )
        if "<!DOCTYPE" in text[:500] or "<!ENTITY" in text:
            raise UpstreamError("ENA XML contains a DTD; refusing to parse", source=SOURCE)
        try:
            root = ET.fromstring(text)  # noqa: S314 - DTDs/entities refused above; size-bounded
    ...
    async def get_sample_metadata(self, accession: str) -> Sample:
        ...
        if classify(acc) != "sample":
            raise InvalidInputError(
                f"{acc} is not an ENA/SRA/BioSample sample accession", source=SOURCE
            )
        ...
        for a in s.findall("./SAMPLE_ATTRIBUTES/SAMPLE_ATTRIBUTE"):
            ...
            attrs.append(
                PhenotypeValue(
                    name=tag, value=a.findtext("VALUE"), unit=a.findtext("UNITS"), source=SOURCE
                )
            )
        ids = {
            "primary": s.findtext("./IDENTIFIERS/PRIMARY_ID"),
            "secondary": [e.text for e in s.findall("./IDENTIFIERS/SECONDARY_ID")],
            "external": [
                {"namespace": e.get("namespace"), "id": e.text}
        ...
        xrefs = [
            {"db": x.findtext("DB"), "id": x.findtext("ID")}
            for x in s.findall("./SAMPLE_LINKS/SAMPLE_LINK/XREF_LINK")
        ]
        taxon = s.findtext("./SAMPLE_NAME/TAXON_ID")
        prov.transformations.append(
            "SAMPLE_ATTRIBUTES copied verbatim as phenotype values "
            "(includes ENA-* housekeeping tags)"
        )
        return Sample(
            accession=s.get("accession") or acc,
            source=SOURCE,
            title=s.findtext("TITLE"),
            description=s.findtext("DESCRIPTION"),
            organism=s.findtext("./SAMPLE_NAME/SCIENTIFIC_NAME"),
            taxon_id=int(taxon) if taxon and taxon.isdigit() else None,
            phenotypes=attrs,
            native={
                "alias": s.get("alias"),
    ...
    # -- retrieval ----------------------------------------------------------------
    async def _sequence_record(self, acc: str) -> tuple[dict, Provenance]:
        """Portal `result=sequence` row: accession, sequence_version, base_count, sequence_md5."""
        base = acc.split(".")[0]
        rows, prov = await self._portal(
            "search",
            {
                "result": "sequence",
                "includeAccessions": base,
                "fields": "accession,sequence_version,base_count,sequence_md5,scientific_name,tax_id,"
                "description,mol_type,last_updated",
            },
        )
        row = next((r for r in rows if r.get("accession") == base), None)
        if row is None:
            raise NotFoundError(f"ENA has no sequence {acc}", source=SOURCE)
        version = str(row.get("sequence_version") or "")
        if "." in acc and acc.split(".", 1)[1] != version:
            raise NotFoundError(
                f"ENA's current version of {base} is {base}.{version}, not {acc}",
                source=SOURCE,
                details={"current_version": version},
            )
        prov.source_record_id = f"{base}.{version}" if version else base
        if version:
            prov.source_version = f"sequence version {version}"
        return row, prov

    async def _sequence_file(self, acc: str) -> SourcePage[FileRef]:
        row, prov = await self._sequence_record(acc)
        base, version = row["accession"], str(row.get("sequence_version") or "")
        versioned = f"{base}.{version}" if version else base
        url = f"{BROWSER}/fasta/{versioned}"
        native = {k: v for k, v in row.items() if v not in ("", None)}
        native["endpoint"] = "browser/api/fasta"
        native["sequence_md5_note"] = (
            "sequence_md5 is ENA's MD5 of the upper-case sequence, not of the FASTA file bytes"
        )
        ref = FileRef(
            uri=url,
            ...
            readiness=Readiness(
                state=ReadinessState.DOWNLOAD_REQUIRED,
                reasons=[
                    "ENA Browser API FASTA record; download it, then build a "
                    ".fai locally (explicit preparation)"
                ],
            ),
            native=native,
        )
        return SourcePage(items=[ref], total=1, provenance=[prov])
    ...
    async def check_file(self, file: FileRef) -> FileRef:
        ...
        if not file.uri.startswith("https://"):
            raise UnsupportedError(
                "only HTTPS ENA files can be range-checked; FTP needs preparation", source=SOURCE
            )
        ...
        if not probe.range_supported:
            ...
            if out.index_uri is None:
                ...
                if not iprobe.range_supported and iprobe.status != 200:
                    ...
                    reasons.append(
                        "range reads verified for file and index; assembly is not reported "
            ...
            if out.format == FileFormat.VCF and out.compression != Compression.BGZF:
                state = ReadinessState.INDEX_REQUIRED
                reasons.append(
                    f"VCF compression is {out.compression}; region queries need BGZF + index"
                )
    ...
    async def fetch_file(
        self, file: FileRef, *, workspace: Path, budget_bytes: int = DEFAULT_FILE_BUDGET
    ) -> Artifact:
        """Download a listed ENA file (and its paired index) over HTTPS within budget; verify MD5s."""
        if not file.uri.startswith("https://"):
            raise UnsupportedError(
                "FTP-only ENA locations need explicit preparation", source=SOURCE
            )
        if budget_bytes <= 0:
            raise InvalidInputError("budget_bytes must be positive", source=SOURCE)
        dest = artifact_path(workspace, file.uri.rsplit("/", 1)[-1])
        dl = await self.files.download(
            file.uri, dest, budget_bytes=budget_bytes, expected_size=file.size_bytes
        )
        ...
        if file.index_uri:
            ...
            if remaining <= 0 or (isize is not None and isize > remaining):
                ...
                raise BudgetExceededError(
                    "the file and its index exceed the transfer budget",
                    source=SOURCE,
            ...
            try:
                idx = await self.files.download(
                    file.index_uri, idest, budget_bytes=remaining, expected_size=isize
                )
        ...
        steps = [
            "streamed within budget",
            "md5 verified against ENA" if md5 else "no ENA md5 available",
        ]
        if index_path:
            steps.append(
                "index md5 verified against ENA" if index_verified else "index has no ENA md5"
            )
        prov = self._prov(
            file.uri, file.accession, "HTTPS download from ENA file server", transformations=steps
        )
        return Artifact(
            path=str(dl.path),
            size_bytes=dl.size_bytes,
            checksums=[
                Checksum(algorithm="md5", value=dl.md5),
                Checksum(algorithm="sha256", value=dl.sha256),
            ],
            checksum_verified=md5 is not None,
            format=file.format,
    ...
    def _verify(dest: Path, dl: Any, md5: str | None, size: int | None) -> None:
        if md5 is not None and dl.md5 != md5.lower():
            dest.unlink(missing_ok=True)
            raise UpstreamError(
                "downloaded ENA file failed its MD5 check; file removed",
                source=SOURCE,
        ...
        if size is not None and dl.size_bytes != size:
            dest.unlink(missing_ok=True)
            raise UpstreamError(
                "downloaded ENA file size differs from ENA's bytes; file removed",
                source=SOURCE,
                details={"expected": size, "observed": dl.size_bytes},
            )

    @operation("transfer_timeout_s")
    async def fetch_sequence_fasta(
        self, accession: str, *, workspace: Path, budget_bytes: int = DEFAULT_FILE_BUDGET
    ) -> Artifact:
        ...
        url = origin.uri  # versioned record URL
        dest = artifact_path(workspace, f"{origin.accession}.fa")
        ...
        if resolved is None or resolved != origin.accession:
            dest.unlink(missing_ok=True)
            raise UpstreamError(
                f"ENA FASTA for {origin.accession} identifies {resolved}", source=SOURCE
            )
        seq_md5, length = await asyncio.to_thread(_sequence_digest, dest)
        want = str(origin.native.get("sequence_md5") or "").lower() or None
        if want and seq_md5 != want:
            dest.unlink(missing_ok=True)
            raise UpstreamError(
                "downloaded sequence does not match ENA's sequence_md5; file removed",
                source=SOURCE,
                details={"expected": want, "observed": seq_md5},
            )
        try:
        ...
        origin.native.update(
            {
                "requested_accession": acc,
        ...
        prov = self._prov(
            ...
            transformations=[
                "downloaded within budget",
                f"resolved {acc} -> {resolved} (ENA sequence_version, FASTA header)",
                "sequence MD5 verified against ENA sequence_md5"
                if want
                else "ENA reported no sequence_md5",
